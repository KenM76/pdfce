...
| **Covers** | `EditSession` end to end: construction, the command/undo/redo model, **all 186 public methods**, the `EditError` taxonomy, the save path (incremental vs full rewrite), the guard/refusal model (encryption, certification, sidecar version, `/Size` suppression), object allocation and byte staging |
...
## 1. Verb index — all 186 public `EditSession` methods

**Count: 186.** Established by brace-matched extraction of the four
