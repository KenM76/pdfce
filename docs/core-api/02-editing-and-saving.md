...
| **Primary subject** | `crates/pdfce-core/src/edit.rs` (20,939 lines) |
| **Covers** | `EditSession` end to end: construction, the command/undo/redo model, **all 108 public methods**, the `EditError` taxonomy, the save path (incremental vs full rewrite), the guard/refusal model (encryption, certification, sidecar version, `/Size` suppression), object allocation and byte staging |
