...
| [ ] | — | [ ] | [x] | Group transparency — `ExtGState` `/BM` blend modes and `/SMask` soft-mask groups (clause 11.3–11.6.4), distinct from the per-image `/SMask`/`/Mask` already shipped. **Disclosed but not implemented**: `render-page` now counts and prints both gaps (113 `/BM` + 36 `/SMask` measured on the Ghent PDF Output Suite's 6 pages) — a `/BM` still composites as `Normal` and an `/SMask` group still paints unmasked; no box above is ticked for this row, because the counting is diagnostics, not the capability itself. |
