...
| [x] | [x] | [ ] | [x] | **CREATE a form field — text, check box, choice** — `add_text_field` / `add_check_box` / `add_choice_field`, CLI `add-text-field` / `add-check-box` / `add-choice-field` (Pass 20.1 PARTIAL `8e799e9`; Pass 20.2 + 20.3 PARTIAL `bca60c9`; both 2026-08-07). Field+widget+`/AP`+`/Annots`+`/Fields` land as ONE undo. Check boxes get a keyed `/AP /N` sub-dict with a **vector-drawn** check (R92 forbids a second, symbolic-font generator); choice fields carry `/Opt` export↔display pairs and Combo/Edit/MultiSelect/Sort, and `Sort` **sorts the array**, not just the flag. Refuses XFA, any certified document (strict gate — creation is structural), **any duplicate field name** (see limits), a degenerate rect, an empty name, `Off`/empty as a check box's on-state, `Edit` without `Combo`, a duplicated `/Opt` export. **Limits: `/MK` borders do NOT paint in pdfce** (R43) — so a check box shows a box and a choice field does not; **same-name MERGE is refused, not performed** (the write-side resolver is unbuilt, which also **blocks radio groups**); **no `/I`//`/TI`, no push buttons, no field/widget DELETION, no field PROPERTY editing** (rename/reflag/move/resize) |
...
| ◐ | ◐ | [ ] | [x] | Form field creation/authoring — **IN PROGRESS since 2026-08-07** on the operator's direct instruction (*"get form field creation/editing done next"*, which answers open question **(m)**). **Text, check box and choice creation have shipped** (core+CLI — moved to *Implemented*): Pass 20.1 PARTIAL, Pass 20.2 + 20.3 PARTIAL. **Now the declared priority (engineer ruling 2026-08-07): the field-path RESOLVER — Pass 20.0's substrate + Pass 20.1's unbuilt half** — because **radio groups are blocked on its merge primitive**, same-name merge is a capability currently refused, and the operator's *"editing"* sits at F6 behind everything. **Still planned:** the resolver + four collision outcomes, all of F0, radio, field/widget deletion, `/I`//`/TI`, push buttons, tab order (F4, blocked on a spec-RAG `/Tabs` dispatch), the GUI surface (F5, needs a UI-specialist dispatch first), field **property editing** (F6). **Two engineer rulings still owed:** F0's disposition (owed/deferred/retired — it was *skipped*, which is not one of the three) and the CLI verb shape (`add-check-box` vs decision 020's `forms add-field --type …`, now three verbs deep) |
