...
| [x] | — | **[ ]** | ? | **Cancellable rendering — a render in progress can be STOPPED, and stopping it stops the work** (`e4256f2`, 2026-08-07; no Pass ID — the ID argument is in `ROADMAP.md`'s *Shipped* entry §6). `RenderCancel` is an `Arc<AtomicBool>` in `pdfce-render` — no windowing, no runtime, no executor, so §3 holds and it works unchanged under wasm. **Measured: pre-cancelled 322 ms against 10,367 ms uncancelled; 28.9 ms from `cancel()` to thread exit mid-render.** `RenderOptions.cancel` defaults to `None`, so **every existing caller keeps an uninterruptible render** and the CLI, the round-trip oracle and the R85 harness cannot acquire a new failure mode from the field existing. Enabled by `ObjectGraph` gaining **`Send + Sync`** — a `pdfce-core` **public API change**, taken now because with no remote and no release the affected implementor set is empty exactly once. **★ `gui` IS `[ ]`, AND THAT IS THE POINT OF THIS ROW: layers 1 and 2 shipped, LAYER 3 WAS DELIBERATELY CUT, and THE GUI STILL FREEZES** — `Arc<EditSession>`, the worker, the channel and the generation counter do not exist. A textbook **R151** core-ahead-of-shell row: the mechanism a worker will need, with no worker. `cli` is `—`: a one-shot batch render has no interactive gesture to cancel it. `Acrobat` is `?` — not sourced from the feature RAG, and not ticked on the strength of an assumption |
