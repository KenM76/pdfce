...
| [x] | [x] | [ ] | [x] | Adopt an existing widget into an `/AcroForm` field — every other authoring verb creates a new widget, so geometry that already arrived correct (e.g. from a page insert) cannot be registered; refuses by name if the annotation isn't a widget, is already claimed, or carries no field identity of its own (`Pass 103.1`). **No GUI surface — `pdfce-gui` work is paused; the consumer is the out-of-tree `pdfceGUI` project.** |
