...
| [x] | [x] | [ ] | [x] | Merge several files into one. **Not reachable in `pdfceGUI`** — `pages.merge_into` is drawn on the Pages tab and falls through to `command-unimplemented`; blocked on a decision (`pageops::insert` returns a whole new document's bytes rather than mutating the open session, so wiring it as-is would discard the undo log), not on the engine. |
