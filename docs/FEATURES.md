...
| [ ] | [ ] | [ ] | [x] | Adopt an existing widget into an `/AcroForm` field — every current authoring verb creates a new widget, so geometry that is already correct cannot be registered (`Pass 103.1`). |
