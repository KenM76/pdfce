...
| [x] | [x] | [x] | [x] | Edit existing text runs in place, including composite/CID (`/Type0`). **Page-stream text only** — a run inside a form XObject (title blocks, dimension callouts on a CAD sheet) is read-only; check `TextRun::editability()` before offering a caret (`Pass 118.0`). |
| [x] | [x] | [x] | ◐ | Text formatting — size, colour, family/style, char and word spacing, horizontal scale, super/subscript, synthetic bold/italic. **Same page-stream-only limit as the row above** (`Pass 118.0`). |
