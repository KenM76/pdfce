...
| [x] | [x] | [x] | ◐ | Text formatting — size, colour, family/style, char and word spacing, horizontal scale, super/subscript, synthetic bold/italic. **Page-stream text only** — unlike the row-in-place edit above, `format_text` was not retargeted into form XObjects this session (`Pass 119.2`, Backlog). |
...
| [ ] | [ ] | [ ] | [x] | Restyle (not just edit) text inside a form XObject — `format_text`, then `reflow_block`/`add_text`, retargeted the way `edit_text` already was (`Pass 119.2`). Page-stream text only today. |
