...
| [x] | [x] | [ ] | [x] | Adopt an existing widget into an `/AcroForm` field, with a pre-flight preview that reports what would happen (or why it would refuse) without committing anything — every other authoring verb creates a new widget, so geometry that already arrived correct (e.g. from a page insert) cannot be registered; refuses by name if the annotation isn't a widget, is already claimed, or carries no field identity of its own (`Pass 103.1`; preview `Pass 103.4`). **No GUI surface — `pdfce-gui` work is paused; the consumer is the out-of-tree `pdfceGUI` project.** |
...
| [x] | [x] | [ ] | [x] | Add a bookmark — `add_outline_item` creates `/Outlines` if absent, appends as the last sibling, maintains `/Prev`/`/Next`/`/First`/`/Last`/`/Count` (`Pass 103.0`). No rename, reorder, delete or re-parent of an existing item yet. |
| [x] | [x] | [ ] | [x] | Author a named destination and point an outline item at it — `add_named_destination` defines a `/Names`/`/Dests` name-tree entry; `add_outline_item` accepts `Destination::Named` (`Pass 103.3`). No rename or delete of an existing named destination yet. **No GUI surface.** |
