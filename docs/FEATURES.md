...
| [x] | [x] | [ ] | [x] | **CREATE a form field** — `add_text_field` / `pdfce-cli add-text-field` (`8e799e9`, 2026-08-07). **Text fields only**; checkbox/radio/choice/pushbutton are *Planned*. Field+widget+`/AP`+`/Annots`+`/Fields` land as ONE undo. Refuses XFA documents, any certified document (strict gate — creating a field is a structural change), a name held by a different field type, a degenerate rect, an empty name. **Limits: the `/MK` border is written but does NOT paint in pdfce** (R43 named-not-painted); **same-name/same-type does not merge** (flagged, unverified); **no field PROPERTY editing** (rename/reflag/move/resize). ⚠ **NO Pass ID assigned** — see `ROADMAP.md`'s *IDENTITY UNRESOLVED* entry at the head of *Shipped* |
...
| ◐ | ◐ | [ ] | [x] | Form field creation/authoring — **STARTED 2026-08-07** on the operator's direct instruction (*"get form field creation/editing done next"*, which answers open question **(m)**). **Text-field creation has shipped** (core+CLI — moved to *Implemented*). **Still planned:** checkbox/radio (F2), choice + pushbuttons (F3), tab order (F4, blocked on a spec-RAG `/Tabs` dispatch), the GUI surface (F5, needs a UI-specialist dispatch first), and field **property editing** — rename/reflag/move/resize — which the operator asked for in the same breath and decision 020 filed only as the non-gating fast-follow F6. **Pass 20.0 (F0, the correctness substrate) was SKIPPED, not built**; decision 020's resolver and its collision branch are also unbuilt. See `ROADMAP.md`'s *IDENTITY UNRESOLVED* entry |
