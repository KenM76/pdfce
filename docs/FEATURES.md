...
| [x] | [x] | [ ] | [x] | Merge several files into one — three verbs share this neighbourhood: `pageops::insert` (CLI `insert-pages`) returns a whole new document's bytes; `EditSession::merge_document` (`Pass 106.0`) merges pages + `/AcroForm` into an open session as one undo entry, whole-source-document only (naming collision tracked as owed item 29). **Still not reachable in `pdfceGUI`** — `pages.merge_into` falls through to `command-unimplemented`; the undo-loss blocker is resolved, only the wiring is missing. |
