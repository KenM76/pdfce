...
| [x] | [x] | [ ] | [x] | Paint shading via the `sh` operator only — axial and radial (types 2–3), per-pixel against the current clip. Ghent PDF Output Suite coverage 0/16 → 14/16; the 2 unpainted are mesh shadings, not this row. **GUI surface deliberately paused by the operator, not yet built.** |
...
| [ ] | — | [ ] | [x] | Paint tiling patterns (`PatternType 1`). Currently deferred; `/Pattern` fills paint nothing, a conforming default per Table 74, not a crash. |
| [ ] | — | [ ] | [x] | Paint shading patterns (`PatternType 2`, named via `scn`). `Shading::load` exists and is reused, but has exactly one caller — the `sh` operator's own handler; `scn` naming a pattern still counts `patterns_unpainted` and draws nothing. Same unpainted-`/Pattern` shape as tiling patterns, but the geometry is base-CTM-relative rather than CTM-relative, so it is not a drop-in reuse of `sh`'s paint path. |
| [ ] | — | [ ] | [x] | `/Separation`/`/DeviceN`/`Lab` colour spaces on image XObjects — the per-pixel path. Vector fills already convert through the same tint-transform evaluator. |
| [ ] | — | [ ] | [x] | Group transparency — `ExtGState` `/BM` blend modes and `/SMask` soft-mask groups (clause 11.3–11.6.4). Distinct from the per-image `/SMask`/`/Mask` already shipped. |
| [ ] | — | [ ] | [x] | Type 3 font rendering and `Tr` 4–7 text-clipping modes. |
| [ ] | — | [ ] | [x] | Overprint simulation. §8.6.7's "ignore if unsupported" fallback applies today, which is conformant; simulating it needs a CMYK compositing buffer. |
