...
| [ ] | [ ] | [ ] | ? | Kind-agnostic object clipboard — copy/cut/paste any selection, cross-document and cross-session, via a versioned `ObjectClip` (`Pass 120.0`–`120.4`). |
...
| [ ] | [ ] | [ ] | [x] | Move, resize and rotate a content-stream object (path, text, image, form/inline-image XObject) via `q…cm…Q` matrix wrapping, kind-agnostic — supersedes plain operand-rewriting (`move_objects`), which can express translation only and cannot spell a rotated rectangle or a scaled stroke width. |
