...
| [ ] | — | [ ] | [x] | Transparency GROUPS — isolated/knockout compositing through an offscreen buffer (clause 11.4–11.6) and `/SMask` soft-mask groups, distinct from the per-image `/SMask`/`/Mask` already shipped. `/Group` is now READ (previously not read at all): 187 groups flattened as an approximation (each object inside painted in place, blend/alpha applied per-object rather than to the group's composited result — wrong wherever the group's own backdrop or internal occlusion order matters) and 47 isolated-or-knockout groups counted separately, both disclosed on `render-page`'s stable line (Ghent PDF Output Suite, 6 pages). No group is composited through a real offscreen buffer in any shell. |
