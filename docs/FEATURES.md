...
| [ ] | [ ] | [ ] | [x] | Bookmark editing — rename, reorder, delete, re-parent an existing outline item; links; named destinations so a carried bookmark resolves (`Pass 103.3`). Create shipped as `Pass 103.0`, see *Implemented*. |
| [ ] | [ ] | [ ] | ? | Page labels carried from a source document on insert — distinct from the target's own tree going stale, which is matched-and-disclosed already; inserted pages' labels currently vanish (`Pass 103.2`, needs an Acrobat measurement). |
