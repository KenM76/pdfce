...
| [x] | [ ] | [ ] | [x] | True in-place page insertion, so Insert edits the open document — `EditSession::insert_pages` (`Pass 99.0`) copies each source page's objects at fresh IDs, remaps every reference, splices into the target's page tree as one undoable command. **Document-level structures are deliberately not merged, unlike `pageops::insert` above — but a page's `/Annots` reaches its widgets, so widgets DO arrive while `/AcroForm` does not: boxes that draw like form fields and that nothing can fill.** It now returns `InsertOutcome { pages_inserted, orphaned_widgets }` and the orphan count is **exact, not an upper bound** (`/AcroForm` is unmerged and every object number is remapped, so no target field can claim an arriving widget) and **permanent, not interim** (a field's widgets can straddle inserted and non-inserted pages, so a residue survives any merge). **No shell wired yet** — the CLI's `insert-pages` is the *other* verb, `pageops::insert`, which does merge `/AcroForm`. |
