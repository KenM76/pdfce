...
| [x] | [x] | [ ] | [x] | Merge several files into one — three verbs share this neighbourhood: `pageops::insert` (CLI `insert-pages`) returns a whole new document's bytes; `EditSession::merge_document` (`Pass 106.0`) merges pages + `/AcroForm`, and now named destinations then the outline/bookmark tree too (`Pass 106.1` — destinations first, so a bookmark pointing at a renamed key is rewritten before the outline copy runs), into an open session as one undo entry, whole-source-document only (naming collision tracked as owed item 29). **Page labels and `/OCProperties` still aren't carried** — see *Planned*. **CLI runs the full merge but doesn't print the three new counts** (`named_destinations_carried`/`renamed`, `outline_items_carried` exist on `MergeOutcome`, absent from `merge-document`'s output line) — a disclosure gap, not a merge gap. **Still not reachable in `pdfceGUI`** — `pages.merge_into` falls through to `command-unimplemented`; the undo-loss blocker is resolved, only the wiring is missing. |
