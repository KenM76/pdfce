...
| [x] | [x] | [x] | **[ ]** | Groups carrying scale, number format (decimal/fraction) and ANSI/ISO drafting standard. A group can be created and its scale, standard and style set; its **unit** can be changed only as a side effect of `set_group_scale` (which carries a `NumberFormat`), its **name** cannot be changed at all, it cannot be deleted, and a placed ce dimension cannot be moved between groups — see the two Planned rows below. |
...
| [x] | [x] | [ ] | ? | **Build provenance stamp** — `pdfce-cli --version` states the UTC build time, the `git describe` revision **including a `-dirty` marker**, the UTC commit time, and that `iccce` is **not linked**. Degrades to a named `unknown`, never a plausible wrong value. `-V` stays the bare parseable `pdfce-cli 0.7.0`. gui is `[ ]` **not `—`**: `pdfce_core::build::BuildInfo` is available to every shell and the GUI simply does not surface it yet (GUI work paused). **Makes builds non-reproducible by construction** — `SOURCE_DATE_EPOCH` honoured as the escape hatch; whether to use it is open operator question `(bo)`. |
...
| core | cli | gui | Acrobat | Feature |
|:----:|:---:|:---:|:-------:|---------|
| [ ] | [ ] | [ ] | — | **`iccce` revision/date/time in pdfce's version output** — BLOCKED: pdfce does not depend on `iccce` at all (zero matches in every `Cargo.toml` and every `.rs`), so there is no version to report and stamping one would assert a relationship that does not exist. The **shipped** build stamp (*Implemented → Shell & UX*, **not** above — it moved out of this section when `Pass 101.0` shipped 2026-08-18) says plainly that `iccce` is not linked; this row replaces that line the moment the dependency lands. Whether it ever should is open operator question `(bp)`. |
...
| [ ] | [ ] | [ ] | ? | Rename or delete a ce-dimension group. Creating one and setting its scale, standard and style works; there is no session verb for either of these, so a mistyped group name is permanent for the life of the document. The model half already exists (`Group::name`, `DimensionModel::delete_group`). `Group::unit` is **not** the same hole: `set_group_scale` takes a whole `NumberFormat`, which carries the unit, so the unit is writable — but only by also setting a scale the operator may not want to change. That is a naming/discoverability defect, and `set_group_unit` (or a stated ruling that recalibration is the sanctioned route) belongs beside the rename. |
...
| [x] | [ ] | [ ] | [x] | True in-place page insertion, so Insert edits the open document — `EditSession::insert_pages` (`Pass 99.0`) copies each source page's objects at fresh IDs, remaps every reference, splices into the target's page tree as one undoable command. **Document-level structures are deliberately not merged, unlike `pageops::insert` above — but a page's `/Annots` reaches its widgets, so widgets DO arrive while `/AcroForm` does not: measured 13 widgets with `fields=None`, drawn but unfillable, and the count is not reported (`Pass 102.0`).** No shell wired yet. |
