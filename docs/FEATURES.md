...
| [ ] | — | [ ] | [x] | Knockout transparency groups (`/K true`) and `/SMask` soft-mask groups, distinct from the per-image `/SMask`/`/Mask` already shipped and from non-knockout group compositing (now shipped — see *Implemented*). Knockout groups composite as ordinary groups today — correct outer boundary, incorrect internal occlusion order (§11.4.6: each element should composite against the group's INITIAL backdrop, not the accumulated result) — counted via `groups_knockout_approx` (47 on the Ghent corpus) rather than silently wrong; GWG 16.1 still shows its crosses. Soft-mask groups (36 occurrences, Ghent) remain entirely unread. Both blocked on a `pdfce-spec-librarian` dispatch for §11.4.6/§11.5.2–.3, checking whether knockout is representable in pdfce's single-alpha buffer model before implementation is attempted. |
