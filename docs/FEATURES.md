...
| [ ] | — | — | ? | **Interactive-speed rendering of a large CAD drawing** — 1× is **18,870 ms** at `76200e9` and the reference product is *"feels instant"* (uninstrumented; ~1.6 s cold-to-sharp on an M4 Pro). **Scoped, not started; no Pass ID.** Order, and the order is the finding: (1) stop allocating a page-sized mask per clip — `Mask::new` is **10.1 s of the remaining 18 s**, and most clips are `re W n` rectangles needing no mask at all; (2) **re-measure** the 2×-only cache cliff, which may vanish for free; (3) tiling/threading **last** — painting every path is only **0.87 s**, so they would today optimise 5%. `cli`/`gui` `—`: the work is entirely in `pdfce-render`, so **both shells get it with no change of their own** — not "no benefit". **A fork was live on item 1 when this row was filed; re-check state before quoting it** |
