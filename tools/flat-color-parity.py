#!/usr/bin/env python3
"""flat-color-parity — how close is pdfce's COLOUR to the reference engine's,
...
============================================================================
pdfce has three render instruments and none of them measures colour accuracy:
...
For each patch with a reference render:
  ...
  1. Render the patch in pdfce at the reference's **width**, so the two
  ...
  3. ERODE that region (`--erode`, default 3 px). This is the load-bearing
     ...
     pdfce's differ by a row or two — so an un-eroded region would sample
     pdfce's antialiased boundary and report edge noise as colour error. That
  ...
  4. Read pdfce's **median** colour over the eroded region, mapped by
...
=================================
A row says: *at this place, the reference painted this colour and pdfce
...
★ It also does not say WHY. A row showing pdfce 20 counts cool on a grey is
...
spread pdfce put on the same region. This is the specific claim under test:
...
content stream said. A population selected by what pdfce did would be
selected by the thing being measured.

USAGE
=====
    set PDFCE_SUITE_DIR=...        the patch PDFs
    set PDFCE_SUITE_REFS=...       the reference engine's renders, `<stem>.png`
...
`pillow`, `numpy` and `opencv-python`; uses the shipped `pdfce-cli
...
def manifest() -> dict[str, str]:
    """`id -> filename`, read from `pdfce-manifest.txt` beside the corpus.
    ...
    if not root:
        return {}
    path = Path(root) / "pdfce-manifest.txt"
    if not path.exists():
        return {}
    out: dict[str, str] = {}
...
def media_box(pdf: Path) -> tuple[float, float]:
    ...
    here would mean this script could fail on a file `pdfce-cli` renders
    perfectly well.
    """
    b = pdf.read_bytes()
...
def sample(pdfce: np.ndarray, mask: np.ndarray) -> tuple[tuple[int, int, int], float]:
    """pdfce's median colour over `mask`, mapped by NORMALISED coordinates.

    Returns the median and the fraction of sampled pixels that equal it — the
    `flat=` column. A fraction near 1.0 means pdfce painted a flat region
...
def main() -> None:
    ...
    ids = manifest()
    refs = os.environ.get("PDFCE_SUITE_REFS")
    corpus = os.environ.get("PDFCE_SUITE_DIR")
    if not ids or not refs or not corpus:
        sys.exit(
            "SKIP: set PDFCE_SUITE_DIR (patches + pdfce-manifest.txt) and "
            "PDFCE_SUITE_REFS (reference renders). The corpus is not "
    ...
    for pid in sorted(ids):
        ...
        if not render(pdf, out, scale):
            print(f"SKIP {pid}: pdfce could not render it", file=sys.stderr)
            continue
    ...
    header = "patch\tarea\tref\tpdfce\tdelta\tref_spread\tpdfce_spread\tflat"
    lines = [header]
    ...
    if rows:
        ...
        if args.neutrals:
            summary["pdfce_spread_max"] = max(r["pdfce_spread"] for r in rows)
            summary["pdfce_spread_median"] = int(np.median([r["pdfce_spread"] for r in rows]))
            summary["neutral_in_neutral_out"] = sum(
                1 for r in rows if r["pdfce_spread"] <= args.neutral_tol
            )
