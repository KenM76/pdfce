...
`crates/pdfce-core/examples/collateral_probe.rs`.
...
/LineDimension` annotations pdfce AUTHORS, with their `/PieceInfo` sidecar
...
`assemble()` rather than produced by `pdfce-cli`, for the reason every sibling
...
def sidecar(annot_ref: bytes, ap_ref: bytes) -> bytes:
    ...
    Schema per `crates/pdfce-core/src/dimension/sidecar.rs`: `/Version` 4,
...
def redact_ap_names_pagetree() -> bytes:
    ...
    is referenced by nothing else"*. That is true of marks pdfce authored and
