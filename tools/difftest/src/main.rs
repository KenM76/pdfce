//! # difftest — differential test oracle harness (pdfce-core vs oxidize-pdf)
//!
...
//! this harness to exist *before* pdfce-core's from-scratch parser is
...
//!   "open the PDF_Spec RAG and adjudicate", never "pdfce is wrong".
//! - **Fixture sourcing** (docs/LEGAL.md §5): pdfce-authored synthetic
...
//!
//! ## Current comparison surface (grows with pdfce-core)
//!
//! | pdfce-core milestone | comparison added |
//! |---|---|
...
fn main() -> ExitCode {
    ...
    if args.is_empty() {
        ...
        eprintln!("  Runs each file through pdfce-core and the pinned oxidize-pdf");
...
/// divergence. This is the function each pdfce-core milestone extends
/// (see the module-docs table).
fn compare_file(path: &Path) -> Outcome {
    // ---- pdfce side: full Document load (Pass 1 parser stack) ----
    ...
    let pdfce_doc = pdfce_core::document::Document::from_bytes(bytes);
    let pdfce_version = match &pdfce_doc {
        Ok(doc) => {
            println!("  pdfce: {} object(s) parsed", doc.object_count());
    ...
    // Extra oracle facts worth logging (not yet comparable — pdfce has no
    ...
    // ---- verdict ----
    match (pdfce_version, oracle_version) {
        ...
        (a, b) => {
            println!("  version: pdfce={a:?} oracle={b:?}");
            Outcome::Diverge
        }
    }
}
