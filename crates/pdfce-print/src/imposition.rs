...
pub enum ImpositionError {
    ...
    #[error(
        "a booklet of {pages} pages needs more than {limit} sheets, which pdfce will not lay out"
    )]
    BookletTooLarge {
        /// Pages requested.
        pages: usize,
...
/// hardware crops, which looks exactly like a pdfce bug and is not one.
///
...
pub struct BookletSpec {
    ...
    /// than forced, and flagged as pdfce's own addition.
    pub auto_rotate: bool,
}
...
pub struct PosterSpec {
    ...
    /// pdfce does not invent one either; [`Default`] leaves it at zero and
    /// the shell asks.
    pub overlap_pt: f64,
...
impl PosterSpec {
    ...
    /// pdfce's answer is the one that needs no special case — with it
...
pub fn plan_poster(
    ...
    // on the next — which reads as a pdfce bug rather than as bad input.
