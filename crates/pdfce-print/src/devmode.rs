...
//!
//! ## ★ Why this module exists at all: pdfce used to SYNTHESISE one
//!
//! Until 2026-08-18 `pdfce-print` built its `DEVMODE` from
...
//! The consequences were reported from outside by the `pdfceGUI` shell
...
//! is how "the operator configured stapling and pdfce printed unstapled"
//! happens.
//!
//! So: **the base is always the driver's own**, obtained from
//! `DocumentPropertiesW(… DM_OUT_BUFFER)`, and pdfce overwrites exactly
...
//!    `spool` call that may be a separate `pdfce-cli` invocation. Opaque
...
//! pdfce then writes `dmPaperSize` and leaves `DM_FORMNAME` asserted,
...
//! untouched, not interpreted. pdfce has no operator-facing control for
...
/// Byte offsets of the `DEVMODEW` members pdfce reads or writes.
///
...
/// `dmFields` bits for the members pdfce writes.
///
...
///
/// Used only as a floor for the buffer pdfce allocates, never as an
...
/// The shortest public portion pdfce will accept from a driver.
///
/// One byte past `dmFormName`, which is the last member this crate reads
/// or writes. A `dmSize` below this means pdfce would be writing into
...
/// operator (project rule 4: pdfce chose a value the operator did not
/// type, so it says so).
///
...
/// cannot be requested through `dmPaperLength` at all, and pdfce must
...
impl PaperSelection {
    ...
    /// is a wrong print that looks like a pdfce scaling bug.
    ///
    /// ```
    /// use pdfce_print::PaperSelection;
    ///
...
/// deliberately: the structure is driver-defined and the fields pdfce
...
pub enum ConfigurationError {
    ...
    TooShort {
        ...
        len: usize,
        /// The minimum a `DEVMODE` public portion can be for pdfce to
        /// amend it — see `MIN_PUBLIC_BYTES`.
        need: usize,
    },
    /// `dmSize` is below the last member pdfce reads or writes, so
    /// amending it would write into the driver's private tail.
    #[error(
        "this driver reports a DEVMODE public portion of {dm_size} bytes, below the {need} pdfce needs to set orientation and paper without overwriting the driver's private data"
    )]
...
impl PrinterConfiguration {
    ...
    ///
    /// ```
    /// use pdfce_print::{ConfigurationError, PrinterConfiguration};
    ///
    /// assert!(matches!(
    ...
    /// What this configuration asks the device for, as far as pdfce
    /// models it.
    ///
    /// The disclosure surface (project rule 4). After the operator edits
    /// a configuration in the driver's own dialog, pdfce cannot show
    ...
    /// exactly the structure pdfce used to synthesise for EVERY job:
...
/// What a [`PrinterConfiguration`] asks for, as far as pdfce models it.
///
...
mod tests {
    ...
    /// duplex default the operator never asked pdfce to touch, and a
    /// private tail.
    ...
    /// Not asking for it must not assert a tray at all — otherwise pdfce
