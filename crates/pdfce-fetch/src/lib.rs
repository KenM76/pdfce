...
//!
//! `pdfce-core` and `pdfce-render` are **network-free permanently and
...
//! (`pdfce-cli`, `pdfce-gui`) depend on it while the engine never does.
//!
...
pub struct PinnedArtifact {
    ...
    /// (`text-detection-ssfbcj81.rten`), and pdfce pins by hash instead and
...
///
/// Sized against what pdfce actually fetches — the two `ocrs` models are
