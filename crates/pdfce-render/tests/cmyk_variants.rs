...
//! vector:
//!
//! 1. **Decoded CMYK sample values at named pixels.** pdfce's YCCK
...
//!    `pdfce-render/src/image.rs` — exactly where a well-meaning
...
//! exact investigation (006 §3.3). pdfce matched pdfium on all six
...
use pdfce_core::image_codec::{self, CodecColorModel, CodedImage};
...
fn render(doc: &Document) -> RenderedPage {
    ...
    pdfce_render::render_page(doc, &pages[0], 1.0).expect("renders")
}
...
fn vn_and_v0_render_identically() {
    ...
    // does pdfce (006 §3.4: v0 and vn agree engine-by-engine). The
