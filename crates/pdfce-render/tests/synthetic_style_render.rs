...
//!
//! > **Prerequisite check before starting:** confirm `pdfce-render` honours
...
//!
//! The reason that gate is sharp is R85 (preview-equals-saved): pdfce's
//! synthetic bold and italic are *emission* choices — a `2 Tr` with a stroke
//! width, and a shear premultiplied into the run's `Tm` — and pdfce's own
...
//!   place inside `BT … ET` is legal and why the scoping mechanism pdfce
...
///
/// This case proves the displacement is real in pdfce's own rasterizer — the
