...
//! `pdfce-render` parses and subsets a real donor, `pdfce-core` emits the PDF
...
//!
//! It lives in `pdfce-render`'s tests because that is the only crate that can
//! see both: `pdfce-core` must never depend on `pdfce-render` (the parser
//! stays out of core — decision 021 §3.2, R21), so core's own tests can only
//! ever construct a plan by hand.

use pdfce_core::document::Document;
use pdfce_core::text_edit::addtext::{self, AddTextRequest};
...
fn a_donor_face_becomes_an_embedded_type0_font_in_the_saved_file() {
    ...
    assert_eq!(
        pdfce_core::page_tree::pages(&reparsed)
            .expect("pages")
            .len(),
        1
    );
...
/// `pdfce_core::font_embed`, which asserts it over the emitter's own output
...
fn embedded_add_allocates_at_least_six_new_objects() {
    ...
    let page_id = pdfce_core::page_tree::pages(&doc).expect("pages")[0].id.num;
