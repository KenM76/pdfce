...
//! matches the rendered geometry (cross-check against pdfce-render's own
//! walk on fixtures)."* This is the Z2 "agree by construction" gate — the
//! guarantee that the read-only object model
//! (`pdfce_core::vector::decompose`) and the renderer do not decompose the
...
//!
//! - the object model: [`pdfce_core::vector::decompose_page`] under
//!   `Matrix::IDENTITY`, so page space is PDF user space;
//! - the renderer: [`pdfce_render::interpret::trace_paths`], which runs the
...
fn core_points(subpaths: &[pdfce_core::vector::Subpath]) -> Vec<(f64, f64)> {
    let mut out = Vec::new();
    for sp in subpaths {
