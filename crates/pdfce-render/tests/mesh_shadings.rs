...
//!
//! `crates/pdfce-render/src/mesh.rs` unit-tests the pieces: MSB-first bit
...
//! record rather than in a test: pdfce's mesh differs from Acrobat's by
//! **the same amount** pdfce's plain raster image differs from Acrobat's
...
/// from **device** size (`MSH-A3` is a silence pdfce fills), so the geometry
...
/// awaiting encoding, and pdfce's pixmap stores exactly that. Recorded
...
/// must do — so pdfce paints the triangle and discloses the discard rather
...
/// rainbows nobody authored. pdfce refuses with a named reason instead, and
