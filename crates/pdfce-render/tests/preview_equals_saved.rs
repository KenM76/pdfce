...
//!
//! ## Why it lives here (`crates/pdfce-render/tests/`) and not in `tools/`
//!
...
//! does **not** require a new public `pdfce-cli` subcommand for it, because
...
//! 3. **This crate has both halves.** `pdfce-render` depends on
//!    `pdfce-core`, so `EditSession` (the preview side) and the rasterizer
...
//!   function `pdfce_core::redact::apply_redactions(&Document, &SaveOptions)
...
//!   (`pdfce-cli`'s `redact --apply`). An operator therefore cannot see a
...
use pdfce_render::tiny_skia::Pixmap;
use pdfce_render::{RenderOptions, render_page_with_view};
...
///
/// Incremental save (not `to_full_bytes`) because it is pdfce's **default**
...
/// (Pass 15.1/15.2): the largest single content-stream rewrite pdfce
...
/// [`StreamSource::Split`](pdfce_core::view::StreamSource::Split): on the
...
/// grained content edit pdfce performs.
#[test]
