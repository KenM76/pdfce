...
//!
//! The end-to-end proof of decision 008's R44 (*"any appearance pdfce
...
//! If the authored `/AP` were a private pdfce-only rendering (the R44
...
//!
//! It is a `pdfce-render` integration test because that is where the
//! consuming raster lives — the same self-comparison oracle Pass 6.0
//! uses, now closing the authoring loop.

#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic)]

use pdfce_core::annot_author::{Color, MarkupSpec, StampName, TextAnnotSpec};
...
use pdfce_render::{RenderOptions, render_page_with};
...
fn non_white(pixmap: &pdfce_render::tiny_skia::Pixmap) -> usize {
    pixmap
        .data()
        .chunks_exact(4)
...
fn reddish(pixmap: &pdfce_render::tiny_skia::Pixmap) -> usize {
    pixmap
        .data()
        .chunks_exact(4)
...
fn authored_square_paints_after_save_and_reload_r44() {
    ...
    // Reload as a fresh document — nothing pdfce-private survives this.
...
fn dark(pixmap: &pdfce_render::tiny_skia::Pixmap) -> usize {
    pixmap
        .data()
        .chunks_exact(4)
...
/// fresh document — the R44 "nothing pdfce-private survives" boundary.
fn author_text_and_reload(spec: &TextAnnotSpec) -> (Document, pdfce_core::page_tree::Page) {
    let mut session = EditSession::new(blank_page_doc());
...
fn authored_freetext_paints_glyph_pixels_after_reload_r44() {
    ...
    // pdfce-authored variable-text appearance — the R44 proof for text.
...
fn bare_standard14_font_dict_renders_with_no_embedded_program() {
    ...
    let annots = pdfce_core::annot::page_annotations(&reloaded, page.id);
    let ap_id = match &annots[0].appearance {
        pdfce_core::annot::Appearance::Normal {
            stream_id: Some(id),
        } => *id,
...
fn resolve_font_dict<'a>(
    doc: &'a Document,
    ap_dict: &'a pdfce_core::object::Dict,
) -> &'a pdfce_core::object::Dict {
...
fn authored_sticky_note_marker_paints_but_popup_does_not() {
    let (reloaded, page) = author_text_and_reload(&TextAnnotSpec::Sticky {
        ...
        icon: pdfce_core::annot_author::StickyIcon::Note,
...
fn authored_highlight_paints_with_multiply_and_survives_reload() {
    use pdfce_core::annot_author::{Quad, TextMarkupKind};
