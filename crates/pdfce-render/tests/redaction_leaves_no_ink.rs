...
//! pdfce removes is placed so that it crosses the region — a thick diagonal
...
//!
//! This test lives in `pdfce-render`'s suite because `pdfce-core` cannot
...
fn the_redacted_region_renders_white_and_the_rest_still_paints() {
    ...
    if let Ok(dir) = std::env::var("REDACT_DEBUG_DIR") {
        ...
        let cs = pdfce_core::content::ContentStream::from_page(&d.view(), &p).unwrap();
