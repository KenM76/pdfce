...
//! dependencies pinned to tag `v0.3.0`. `pdfce-core`'s build script went on
//! emitting the literal string `not-linked-yet`, so
//! `pdfce-cli --version` told the operator that pdfce does not link `iccce`
...
//! occurred.
//!
//! ## ★★ Why this test lives HERE, in `pdfce-render`
//!
...
//!
//! `pdfce-core` could not host this: it does not depend on `iccce` and never
//! will, which is exactly the structural gap that let the defect live.
//! `pdfce-cli` could, but it reaches `iccce` transitively, so the proof
...
use pdfce_core::build::BuildInfo;
