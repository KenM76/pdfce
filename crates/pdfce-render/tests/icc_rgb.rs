...
use pdfce_render::{InkProbeSource, RenderOptions, RenderedPage};
...
fn render(name: &str, options: &RenderOptions) -> RenderedPage {
    ...
    pdfce_render::render_page_with_view(&doc.view(), &pages[0], SCALE, options).expect("renders")
}
...
fn a_decoder_call_without_a_context_still_falls_back_to_the_device_space() {
    ...
    let xobjects = resources
        ...
        .and_then(pdfce_core::object::Object::as_dict)
        .expect("XObject dict");
    let im0 = xobjects.get(b"Im0").map(|o| view.resolve(o)).expect("Im0");
    let pdfce_core::object::Object::Stream(stream) = im0 else {
        panic!("Im0 is a stream")
    };
    let raw = view.slice(stream.data_span).expect("stream bytes");
    let decoded = pdfce_render::image::decode(
        &view,
        &stream.dict,
        raw,
        resources,
        pdfce_render::gstate::Rgb::BLACK,
        pdfce_render::image::ImageOrigin::XObject,
        pdfce_render::RenderPolicy::default(),
        pdfce_render::image::IccContext::unmanaged(),
    )
    .expect("decodes");
