...
//! | stage | whose |
//! |---|---|
//! | composites the page **in ink** | pdfce's |
//! | converts the composited result **to sRGB on the way out** | `iccce`'s |
//!
//! — and pdfce's only switch for the buffer, `--max-cmyk-buffer-bytes`, turns
...
/// between pdfce's green and a reference engine's is downstream of this point
