...
//! what this file asserts. `pdfce-core`'s `tests/ocr_layer.rs` proves the text
//! goes in and comes back out; this proves the other half, that putting it in
//! cost nothing visible.
//!
//! ## Why it has to live in `pdfce-render` and not beside the writer
//!
//! `pdfce-core` cannot rasterise — it has no renderer and, by the GUI-core
...
use pdfce_core::ocr::layer::{OcrLayerOptions, add_ocr_layer};
use pdfce_core::ocr::{OcrPage, RecognizedWord};
use pdfce_core::page_tree::{self, Rect};
use pdfce_render::render_page_view;
