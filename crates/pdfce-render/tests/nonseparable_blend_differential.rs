//! ★ The differential that decision 066 asks for, pointed at pdfce's own code.
//!
//! Decision 066 says pdfce verifies a spec-governed computation against the
...
//! pdfce's own replacement of that dependency, so this file applies it
//! inward.
//!
...
//!   in either implementation, so **pdfce and the crate must AGREE**.
//! - Where `SetLum` drives a channel **below zero**, the crate clamps and
//!   pdfce rescales, so they **must DIFFER**.
//!
...
use pdfce_render::blend_nonsep::{NonSeparableBlend, blend};
use pdfce_render::tiny_skia::{BlendMode, Paint, Pixmap, PixmapPaint, Transform};
...
fn tiny_skia_blend(mode: BlendMode, cb: [f32; 3], cs: [f32; 3]) -> [f32; 3] {
    ...
    dst.fill_rect(
        pdfce_render::tiny_skia::Rect::from_ltrb(0.0, 0.0, 1.0, 1.0).unwrap(),
        &paint,
        Transform::identity(),
        None,
    );
...
///
/// Computed from pdfce's own definition, which is fair here: the question is
...
/// ★ WHERE THE DEPENDENCY IS CORRECT, pdfce AGREES WITH IT.
///
/// This is the half that catches "wrong in a new way". If pdfce had
...
#[test]
fn pdfce_agrees_with_tiny_skia_wherever_clip_color_cannot_fire() {
    ...
    assert!(
        worst.0 <= 3.0 / 255.0,
        "pdfce and tiny_skia must agree where ClipColor cannot fire, but they \
         differ by {:.1}/255 on {}. A difference HERE means pdfce's \
...
/// ★ WHERE THE DEPENDENCY IS BROKEN, pdfce DIFFERS FROM IT — and differs in
...
/// `ClipColor`'s low-gamut rescale should fire, pdfce must not match a
/// per-channel clamp.
#[test]
fn pdfce_differs_from_tiny_skia_exactly_where_the_dependency_is_wrong() {
    ...
    assert!(
        differing > 0,
        "pdfce matched tiny_skia on ALL {out_of_gamut} out-of-gamut samples. \
         That means the ClipColor rescale is not firing and pdfce has \
...
fn the_canonical_case_is_still_a_divergence() {
    ...
    assert!(
        ...
        "the standard requires black here; pdfce gave {mine:?}"
    );
...
fn centre_pixel(bytes: Vec<u8>) -> (u8, u8, u8) {
    let doc = pdfce_core::document::Document::from_bytes(bytes).expect("fixture parses");
    let page = pdfce_core::page_tree::pages(&doc)
        .expect("page tree")
        .remove(0);
    let r = pdfce_render::render_page(&doc, &page, 1.0).expect("renders");
