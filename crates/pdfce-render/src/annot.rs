...
pub enum AnnotationClass {
    ...
    ///
    /// A `/Subtype` pdfce has never heard of is, by construction, not one
...
impl AnnotationClass {
    ...
    /// anything pdfce infers: the partition is normative, so a subtype
    ...
    /// and pdfce does not repair it into one — it classifies as
    /// [`Self::Other`] and the operator sees it under whatever scope admits
    /// non-markup annotations, rather than pdfce quietly deciding what the
    /// producer meant.
    #[must_use]
    pub fn of_subtype(subtype: &[u8]) -> Self {
        match subtype {
            ...
            // markup, rather than printing because pdfce had never heard of
            // it.
            //
...
/// render paints.
///
/// # Why this exists (Acrobat parity, and what pdfce could not say before)
///
...
///
/// pdfce could express exactly one of those four ("Document and Markups",
...
/// So the enum is five-valued: the four Acrobat scopes, and pdfce's
/// content-only raster.
///
...
pub enum AnnotationScope {
    ...
    ///
    /// Not one of Acrobat's four; it is pdfce's own, and it is what
    ...
    /// empty" from "pdfce was told not to draw the page".
    FormFieldsOnly,
}
...
mod tests {
    ...
    fn form_fields_only_paints_widgets_and_suppresses_page_content() {
        ...
        assert_eq!(
            ...
            "pdfce did not read the content streams, so it reports nothing about them"
        );
    }
    ...
    ///
    /// `with_annotations(false)` is a live caller contract (`pdfce-cli`'s
    ...
    /// merely because pdfce has not heard of it. Classifying an unknown
