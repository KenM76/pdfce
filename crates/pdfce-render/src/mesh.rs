...
//! that is **pdfce's own** rather than the standard's is visibly marked.
//!
...
//!   [`pdfce_core::settings::MeshPatchPadding`], defaulting to
...
//!   left open** ("may be linear or nonlinear"). pdfce interpolates
...
//!   `/SM` bounds *colour* error rather than geometric deviation. pdfce
...
//!   indices** and produce palette-order rainbows. pdfce refuses it with a
...
//!   first patch whose flag is nonzero, the standard is silent. pdfce
...
use pdfce_core::settings::MeshPatchPadding;
...
/// the coverage test - pdfce's own choice, and the fix for a defect that is
...
pub struct ParseInput<'a> {
    ...
    pub intent: pdfce_core::settings::CmykIntent,
}
...
///   the stream length, and pdfce discloses the inference and any partial
...
fn parse_type5(
    ...
    if !verts.len().is_multiple_of(k) {
        // A partial final row. MSH-N2 leaves this undefined; pdfce keeps
...
fn parse_patches(
    ...
    // alone), so pdfce keeps what completed and says that it did.
    if r.remaining() > 0 {
        *truncated = true;
    }
...
/// Cells per side to subdivide one patch into — `MSH-A3`, pdfce's own
...
/// shading was paintable and pdfce did not paint it.
///
...
fn fill_triangle(
    ...
    for py in lo_y..hi_y {
        ...
        for px in lo_x..hi_x {
            ...
            // pdfce's choice and is what "Gouraud" names.
