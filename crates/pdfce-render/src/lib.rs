//! # pdfce-render — headless rasterizer (draw-ops → pixels)
//!
...
//! is also what makes the future web fork and `pdfce-cli render-page`
...
//! `pdfce-cli render-page` writes them to PNG; the WASM fork reuses
...
// path; `pdfce-cli` reaches `Pixmap::encode_png` through it.
pub use tiny_skia;
...
/// `pdfce-render` itself never enumerates, opens, or reads a font
...
pub fn render_page_with_view(
    ...
    // by `options.annotations`, so `--no-annotations` still discloses how
...
mod tests {
    ...
    /// in `pdfce-core` (decision 004 §7). `722` is Helvetica's real AFM
    /// advance for `H`.
    ...
    fn std14_without_widths_uses_the_afm_metrics() {
        ...
        // `pdfce_core::fontdata` — keyed by GLYPH NAME, so this
    ...
    /// pdfce does not bundle), with explicit `/Widths` so the pen advance
    ...
    fn a_32_deep_form_chain_renders_because_the_corpus_contains_one() {
        ...
        // pdfce's guard is pure policy and has to clear the corpus.
        //
    ...
    fn icc_based_image_falls_back_to_the_component_count() {
        // color__iccbased.md: pdfce does not parse ICC profiles, so the
        ...
        let (doc, page) = doc_with_extra_objects(
            ...
            &[
                ...
                // A minimal ICC stream: pdfce only ever reads its /N.
    ...
    /// The synthetic JPEG codestreams from `pdfce-core`'s DCT tests,
    ...
    /// The synthetic CCITT/JBIG2 codestreams from `pdfce-core`'s bilevel
    ...
    /// The synthetic JPEG 2000 codestreams from `pdfce-core`'s JPX
    /// tests, pulled in the same way and for the same reason as
    ...
    fn jpx_image_mask_honours_decode_and_thresholds_the_delivered_depth() {
        ...
        // codestream to carry 1-bit samples, but pdfce normalizes every
    ...
    fn inline_image_ascii_hex_renders_through_the_same_path() {
        ...
        // out of the Table 93/94 abbreviations by pdfce-core — so this
    ...
    //
    // A within-block reflow (`pdfce_core::text_edit::apply_reflow`, Pass
    ...
    fn reflow_then_render(
        content: &str,
        width: f64,
        align: Option<pdfce_core::text_edit::BlockAlignment>,
    ) -> (u32, u32, u32, u32, Diagnostics) {
        use pdfce_core::text_edit::{ReflowRequest, apply_reflow};
        ...
        let pages = pdfce_core::page_tree::pages(&doc2).expect("pages");
    ...
    fn justified_reflow_ink_reaches_the_box_right_margin() {
        ...
        // positions are asserted in pdfce-core).
        let (_x0, _y0, x1, _y1, diag) = reflow_then_render(
            "BT /F1 10 Tf 5 80 Td (aa bb) Tj ET\n\
             BT /F1 10 Tf 5 70 Td (cc dd end) Tj ET\n",
            60.0,
            Some(pdfce_core::text_edit::BlockAlignment::Justified),
        );
