//! # The `pdfce-core` font-data seam
//!
//! `pdfce-render` names every standard-14 metric, standard-14
//! descriptor and Annex D encoding lookup through **this** module
//! rather than through `pdfce_core::fontdata` directly. The indirection
...
//! call site.
//!
//! ## Why the data lives in `pdfce-core` and not here
//!
//! Decision 004 §7 splits the font problem in two:
//!
//! | Half | Owner | Needed by |
//! |---|---|---|
//! | code → glyph name → Unicode; standard-14 widths and descriptors | **`pdfce-core`** (`fontdata`) | text **extraction**, `ToUnicode` synthesis, form-field appearance generation — none of which rasterize anything |
//! | glyph name / GID → outline | **`pdfce-render`** ([`super::program`]) | rasterization only |
//!
