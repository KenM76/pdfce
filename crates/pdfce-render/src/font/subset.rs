...
//! # Where this sits
//!
//! `pdfce-core` defines [`FontEmbedPlan`] and emits PDF objects from it; this
...
//! lives in this crate. Putting a subsetter in `pdfce-core` would give a
...
//! bytes pdfce did not produce. Everything here is bounded and fallible:
...
use pdfce_core::font_embed::{DescriptorMetrics, FontEmbedPlan, OutlineKind, SubsetGlyph};
use skrifa::raw::TableProvider as _;
use skrifa::raw::types::GlyphId16;
use skrifa::{FontRef, MetadataProvider};
use subsetter::GlyphRemapper;

/// Largest donor font file pdfce will parse, in bytes.
///
...
/// can exceed 100 MB; that refusal is honest — pdfce takes one face index
...
pub enum SubsetError {
    ...
    #[error(
        "this font file is {size} bytes, above the {limit}-byte limit pdfce will read. If this is \
         a font collection (.ttc), point pdfce at a single face instead."
    )]
    TooLarge { size: usize, limit: usize },
    /// The bytes are not a font pdfce recognises.
    #[error("this file is not a font pdfce can read (it is not a TrueType or OpenType file)")]
    NotAFont,
    /// The face parsed, but its outlines are not `glyf`.
    ///
    /// Kept distinct from [`Self::NotAFont`] because the operator's next move
    /// differs completely: a CFF face is a *valid* font pdfce cannot yet
    /// embed, and telling them it is "not a font" would be a lie that costs
    /// them time proving otherwise.
    #[error(
        "this font uses CFF (PostScript) outlines, and pdfce can currently embed only TrueType \
         outlines. The font is fine — pdfce's support for this kind is not. Choose a TrueType \
         (.ttf) face."
    )]
    CffNotSupported,
    ...
    #[error("this font uses a feature pdfce's subsetter does not implement yet ({detail})")]
    ...
    #[error(
        "pdfce's font subsetter hit an internal error on this face ({detail}). This indicates a \
...
pub fn plan_subset(
    ...
    // Bound BEFORE parsing. The ceiling exists to stop pdfce reading a file
...
mod tests {
    ...
    #[test]
    fn cff_refusal_says_the_font_is_fine_and_pdfce_is_not() {
