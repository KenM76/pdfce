...
//!
//! # Why pdfce computes these itself instead of calling the rasteriser
//!
...
//!
//! That measurement produced `ARCHITECTURE.md` §12 **decision 066** — pdfce
...
/// pdfce computes, and keeping them in their own type makes it impossible to
...
/// (module docs; `ARCHITECTURE.md` §12 decision 066), so pdfce rasterises the
...
/// and `B(Cb, Cs)` when the backdrop is opaque, which it is here: pdfce's
