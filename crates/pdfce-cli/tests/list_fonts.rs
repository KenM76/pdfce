//! # `pdfce-cli list-fonts` integration tests
//!
//! Black-box tests over the **real binary** for the font inventory that
//! Pass 67.0 phase A ships. `pdfce_core::fontinfo`'s own unit tests pin the
...
//!    does not: when a font's program cannot safely be removed, pdfce says
...
/// facts stated, because a batch sweep needs to tell "pdfce refused" from
...
/// Without the flag, "this document has no fonts" and "pdfce could not
