...
//!
//! `pdfce-core`'s own unit tests pin `resolve_style`'s arithmetic. They cannot
...
//!
//! Everything here is a **ce dimension** — the ones pdfce authors. No **pdf
//! dimension** (a CAD-exported one already in the page content) is touched by
//! any of this; a style is a property of pdfce's own annotation, and a
//! CAD-exported dimension's appearance is page content pdfce must not alter.
...
fn temp_out(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("pdfce-dimension-style-tests");
...
/// pushed into it (`SolidWorks_Dimensions` §F.4). pdfce's cascade is a live
