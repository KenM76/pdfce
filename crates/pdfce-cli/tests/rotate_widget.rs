...
//! 5. **Where pdfce cannot redraw, it says so** — the disclosure is not
...
fn temp_out(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("pdfce-rotate-widget-tests");
...
/// as raw text.
///
/// A byte scan rather than a parse, deliberately: this asserts what pdfce
...
/// one.** So it answers *"did pdfce ever write this array"* and cannot answer
...
/// rewrite every appearance in every form pdfce touches, for nothing.
#[test]
...
/// makes, and `pdfceGUI` refused to ship a border control rather than blur it.
#[test]
...
/// is pdfce's product rule, so a caller who passed `-90` and reads back `270`
/// is told that pdfce did that, rather than being left to wonder whether the
/// file had been wrong.
#[test]
...
/// ★★ **Where pdfce cannot redraw the appearance, it SAYS SO.**
///
/// A check box's `/AP` is state-keyed artwork pdfce's text generator does not
...
#[test]
fn a_widget_pdfce_cannot_redraw_is_rotated_and_disclosed() {
...
///
/// A property pdfce can write and cannot read is exactly the asymmetry
/// `pdfceGUI` refused to ship a border control for. `list-fields --widgets`
