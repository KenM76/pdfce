//! # `pdfce-cli resize-annotation` (`Pass 151.0`)
//!
...
//!
//! `crates/pdfce-core/tests/annot_resize.rs` calls `resize_annotation`
...
/// so it is genuinely **foreign** to pdfce — which is what makes the
...
/// Rule 4 in the CLI: the invocation IS the commit, so what pdfce chose *not*
...
fn scale_stroke_width_reaches_the_saved_bytes() {
    ...
    // Parsed as a NUMBER, not compared as a string: pdfce writes reals in a
...
fn allow_appearance_distortion_lifts_a_real_refusal() {
    ...
    assert!(
        err.contains("pdfce did not draw it"),
