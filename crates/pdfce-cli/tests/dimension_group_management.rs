...
//! `pdfce-core` since `Pass 25.7`. Until this Pass **no subcommand reached any
...
//!
//! Everything here is a **ce dimension** and a ce-dimension GROUP — pdfce's
//! own authored objects. No **pdf dimension** (a CAD-exported one already in
//! the page content) is touched: a group is a pdfce construct that exists only
...
fn temp_out(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("pdfce-dimension-group-tests");
