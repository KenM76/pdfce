//! # `pdfce-cli export-dxf` integration tests — the MULTI-PAGE mode and
//! the scale gate (Pass 52.2)
//!
//! Black-box: these spawn the **real binary** (`CARGO_BIN_EXE_pdfce-cli`)
...
//! exists: `crates/pdfce-core/tests/dxf_scale.rs` pins the inference
...
/// the 1 — so `--scale 1` printed *"pdfce does not know what scale the
...
fn an_explicit_scale_of_one_is_not_warned_about() {
    ...
    assert!(
        ...
        "the operator said 1; telling them pdfce does not know the scale is absurd: {}",
        stderr(&out)
    );
}
