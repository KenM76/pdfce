//! # `pdfce-cli`'s bookmark verbs — black-box over the real binary
//!
...
fn temp_path(tag: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("pdfce_bm_{tag}_{}_{n}.pdf", std::process::id()))
}
...
///
/// The operator named one bookmark and three went. In `pdfce-cli` the
