...
//! tests it against bytes *pdfce* chose, which is the pair that has to agree.
//!
...
//! bytes pdfce has always *authored*; returning it from a *reader* would make
//! a properties control display a border the document does not contain, and
//! the operator's first press would write that invention in. `pdfceGUI`
//! refused to ship the control rather than do that, citing pdfce's own
...
fn temp_path(tag: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("pdfce_widget_{tag}_{}_{n}.pdf", std::process::id()))
}
...
fn the_raw_flag_word_is_printed_beside_the_mapping() {
    // So a control can say "these flags are not one of the four pdfce can
