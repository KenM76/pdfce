//! # `pdfce-cli annotate --note / --note-author / --note-date` (`Pass 150.0`)
//!
...
//!
//! **pdfce does not read a clock.** `--note-date` is the caller's PDF date
...
//! and a timestamp pdfce chose would be a value it *inferred* and wrote
...
fn temp_path(tag: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("pdfce_note_{tag}_{}_{n}.pdf", std::process::id()))
}
...
#[test]
fn the_help_says_pdfce_will_not_supply_the_date() {
    // The reason is operator-facing, not just an internal doc comment: a
    // caller who does not know pdfce refuses to guess will assume it guessed.
