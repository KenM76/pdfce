//! # `pdfce-cli embed-font` integration tests
//!
...
//! `pdfce_core::font_embed_missing`'s own unit tests pin the planning and the
...
//!    anything — it does not — but because it is an *inference*: pdfce is
...
//!    `bundled` means one of pdfce's own faces. That is the disclosure rule 4
...
//!    showed only pdfce's successes would omit. A file where three of seven
...
fn restricted_dir() -> PathBuf {
    let dir = std::env::temp_dir().join("pdfce-embed-restricted");
...
fn temp_out(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("pdfce-embed-tests");
    std::fs::create_dir_all(&dir).expect("temp dir");
    dir.join(name)
}
...
/// warning is on stderr. Rule 4: pdfce is choosing a font the document did
...
fn with_no_font_source_every_font_is_refused_by_name() {
    ...
    assert!(
        ...
        "the refusal must name where to point pdfce:\n{stdout}"
    );
    assert!(
...
/// A bare `ZapfDingbats` takes pdfce's own bundled face, because pdfce writes
