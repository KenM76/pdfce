//! # `pdfce-cli set-info` / `rotate-page` integration tests
//!
...
//! functions and `pdfce-core`'s own tests cover the command log; what
//! *this* file protects is the part a script depends on and a refactor
//! cannot see.
//!
//! ## Why the same key test appears here as well as in pdfce-core
//!
//! `edit → undo → save is byte-identical` is verified in
//! `pdfce-core/tests/edit_undo.rs` at the API level. It is verified
...
fn rotate_page_appends_a_revision_touching_exactly_one_object() {
    ...
    let doc = pdfce_core::document::Document::from_bytes(saved).unwrap();
    let pages = pdfce_core::page_tree::pages(&doc).unwrap();
...
fn rotate_page_relative_accumulates() {
    ...
    let doc = pdfce_core::document::Document::from_bytes(std::fs::read(&twice).unwrap()).unwrap();
    assert_eq!(pdfce_core::page_tree::pages(&doc).unwrap()[0].rotate, 180);
}
...
fn set_info_edits_the_information_dictionary_only() {
    ...
    let Some(pdfce_core::object::Object::String(title)) = dict.get(b"Title") else {
        panic!("title missing");
    };
    assert_eq!(title, b"Quarterly Report");
    // R41: editing metadata does not stamp pdfce's own producer.
    let Some(pdfce_core::object::Object::String(producer)) = dict.get(b"Producer") else {
...
fn set_info_creates_a_dictionary_when_the_file_has_none_and_says_so() {
    ...
    let doc = pdfce_core::document::Document::from_bytes(std::fs::read(&output).unwrap()).unwrap();
...
fn non_ascii_metadata_survives_the_command_line() {
    ...
    let dict = doc
        ...
        .unwrap()
        .clone();
    let Some(pdfce_core::object::Object::String(bytes)) = dict.get(b"Title") else {
        panic!("title missing");
    };
    assert_eq!(
        pdfce_core::edit::decode_text_string(bytes).text,
        "Été — 日本語"
    );
}
