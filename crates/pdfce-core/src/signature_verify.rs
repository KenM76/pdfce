...
//! that drove this module (pdfceGUI, 2026-09-03) asked in as many words that
...
//! notes say which.
//!
//! The verdict vocabulary here is pdfce's own. ETSI EN 319 102-1 defines
...
pub enum Integrity {
    ...
    SignatureInvalid,
    /// pdfce could not reach a verdict, and says why: a subfilter or
...
fn verify_dict<G: ObjectGraph + ?Sized>(
    ...
    match sf {
        ...
        other => {
            verdict.integrity = unverifiable(&format!(
                "the /SubFilter {other:?} is not one pdfce verifies (adbe.pkcs7.detached, ETSI.CAdES.detached, adbe.pkcs7.sha1)"
            ));
            return verdict;
        }
    }
    ...
    let Some(sd) = cms::parse_signed_data(outer.raw) else {
        verdict.integrity = unverifiable("the /Contents is not a CMS SignedData pdfce can read");
        verdict.notes = notes;
        return verdict;
    };
    ...
    let Some(hash) = hash_for(&signer.digest_alg.oid) else {
        verdict.integrity = unverifiable(&format!(
            "the digest algorithm {} is not one pdfce computes (SHA-1/256/384/512)",
...
/// is one pdfce does not implement.
fn check_signature(
    key: &PublicKey<'_>,
    alg: &cms::AlgId<'_>,
    hash: Hash,
    ...
    match (key, alg.oid.as_str()) {
        ...
        (
            ...
            let Some(curve) = Curve::from_oid(curve_oid) else {
                return Err(format!(
                    "the certificate's curve {curve_oid} is not one pdfce verifies (P-256, P-384)"
                ));
            };
        ...
        (PublicKey::Other(k), _) => Err(format!(
            "the certificate's key algorithm {k} is not one pdfce verifies (RSA, EC)"
        )),
        (_, other) => Err(format!(
            "the signature algorithm {other} is not one pdfce verifies"
        )),
    }
}
