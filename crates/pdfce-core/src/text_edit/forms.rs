...
//! — none of this concerns pdfce-authored *ce dimensions*). ISO 32000-1
...
//!    invocation. That is a confirmed, permanent negative result in pdfce's
...
//!    the standard gives pdfce no basis to pretend otherwise.** An editor that
...
///
/// pdfce therefore orders the chain **own → page → enclosing form**, so the
...
/// running it:
///
/// 1. **`pdfce-render`'s interpreter does not do it.** Its `Do` handler takes
...
/// write.
///
/// ## One divergence from `pdfce-render`, stated rather than hidden
///
/// At depth 0 — a form invoked straight from the page — "the page's" and "the
/// caller's" are the same dictionary, so pdfce's three components agree
...
pub struct FormScan {
    ...
    /// pdfce limit, not a file defect: neither edition states any nesting
...
///
/// **64, and it is a pdfce invariant rather than a conformance rule** — the
...
/// **conforming** and pdfce still refuses it; that must be disclosed as
/// pdfce's limit, never as the file's defect.
///
...
/// find: the whole point of that field is that a form pdfce failed to see is
