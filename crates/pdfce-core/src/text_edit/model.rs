...
//! what lets a future `pdfce-gui` canvas tool and the `pdfce-cli` inspector
...
pub struct BlockDiagnostics {
    ...
    /// order (same shape as `pdfce-render`'s and Pass 4's note lists).
...
impl<'a> EditableTextModel<'a> {
    ...
    /// use pdfce_core::{page_tree, text_extract, text_edit};
    ///
