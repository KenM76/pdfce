...
//!
//! **Synthesis** is what happens when no such face resolves: pdfce fakes the
...
//! count in the content stream. pdfce's provenance model maps *bytes to
...
//!
//! Decision 019 §3.6 chose **P-selfevident**. pdfce writes **nothing** into
...
//! anything pdfce-specific — it recognizes **other producers'** faux styles
...
//! add bytes to a file for pdfce's sole benefit, be invisible to every other
...
/// The tangent of pdfce's oblique angle, **12°**.
///
...
///
/// This is pdfce's own documented choice, **not** a parity claim: Acrobat's
...
///
/// Like [`OBLIQUE_TAN`] this is pdfce's own choice. It is exposed as a public
...
/// type is what makes "what pdfce did" and "what pdfce found on reload"
...
///
/// ```
/// use pdfce_core::text_edit::synth::bold_stroke_width;
///
...
/// Premultiply pdfce's oblique shear into a text matrix (§8.3.3).
///
...
///
/// ```
/// use pdfce_core::text_edit::synth::{OBLIQUE_TAN, shear_into};
///
...
/// **Re-detect** a synthesized style from the bytes alone — pdfce's own and
/// other producers' (decision 019 §3.6, P-selfevident).
///
/// This is the whole of the persistence story. No marker was written into the
/// PDF, so on reload pdfce infers the synthesis from the two pieces of
...
/// override*, never a fact pdfce acts on silently.
///
...
///
/// ```
/// use pdfce_core::text_edit::synth::{StyleSynthesis, detect};
///
/// // A run pdfce faux-bolded: mode 2, a 0.264 pt stroke on 12 pt Helvetica.
/// assert_eq!(
...
///
/// Set at twice [`BOLD_STROKE_RATIO`], which gives pdfce's own output ample
...
///
/// Well below pdfce's own [`OBLIQUE_TAN`] so its output is always detected,
...
mod tests {
    ...
    /// The whole persistence story in one test: what pdfce emits is what
    /// pdfce detects, with no marker in between.
    #[test]
    fn pdfce_own_emission_round_trips_through_detection() {
        let size = 14.0;
    ...
    fn another_producers_faux_bold_is_detected_too() {
        // A stroke ratio of 1.5% — not pdfce's 2.2%, but the same technique.
        assert_eq!(
            detect(
                "ABCDEF+Arial",
                2,
