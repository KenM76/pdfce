...
/// tests, `pdfce-cli`'s `reflow` preview path, and `pdfce-gui`'s Pass 15.2
...
impl ReflowRequest {
    ...
    ///
    /// The struct is `#[non_exhaustive]`, so callers outside `pdfce-core`
    ...
    ///
    /// ```
    /// use pdfce_core::text_edit::{BlockAlignment, ReflowRequest};
    ///
