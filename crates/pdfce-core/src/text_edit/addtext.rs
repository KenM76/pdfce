...
//! not an error: pdfce reports *"boxed text overflows the box by N line(s)"*
//! and emits **every** line as real recoverable page content at its true
//! position. If growth additionally passes the page cropbox bottom, pdfce also
...
//! reader's built-in std-14, §9.6.2.2). pdfce emits the **full `/Widths`
...
//! using a complete font descriptor"*), and pdfce already owns the AFM widths
...
//!
//! Because pdfce *chooses* the encoding and *controls* the font, this is the
...
//! is emitted as plain **untagged** page content and pdfce **discloses** that
//! the structure tree / reading order was not updated and no tag was created.
//! pdfce never fabricates a mis-placed structure element (rule 4 / R73);
//! minimal StructTree insertion for new content is the deferred FF-H.
//!
//! ## GUI-core separation
//!
//! Everything here is `pdfce-core`: it takes plain page-space coordinates and a
...
pub enum FontProvenance {
    /// A bundled pdfce Standard-14 substitute face (the default).
    Bundled,
...
impl AddTextRequest {
    ...
    ///
    /// ```
    /// use pdfce_core::text_edit::{AddTextRequest, BlockAlignment};
    ///
...
pub enum AddTextError {
    ...
    /// enforcement a `shall`, so pdfce refuses rather than silently breaking
...
/// use pdfce_core::text_edit::{add_text, AddTextRequest};
///
...
/// pdfce can perform — so pdfce refuses rather than performing the edit and
...
/// type, no `&Document`, no mutation — so it lives in `pdfce-core` and the
...
/// use pdfce_core::text_edit::{preview_wrap, BlockAlignment};
///
