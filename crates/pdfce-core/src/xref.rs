...
//!    [`STARTXREF_SCAN_WINDOW`] is pdfce policy, not spec).
...
//!    cycle guard ([`MAX_XREF_SECTIONS`], pdfce policy).
...
//! **direct** objects. pdfce relies on that: the stream is parsed with
...
//! interpreted as a reference to the null object" — pdfce records those
...
//! `SP LF`, `CR LF`. Real-world 19/21-byte deviants exist; pdfce
...
/// `startxref` keyword.
///
/// pdfce policy, not spec: §7.5.5 places `startxref` on the
...
///
/// pdfce policy (ARCHITECTURE.md §10): each legitimate section is one
...
///
/// pdfce policy (ARCHITECTURE.md §10.1): bounds allocation against a
...
///
/// pdfce policy: the spec puts no ceiling on `W`'s elements, but a
...
///
/// pdfce policy: bounds the per-row read for a hostile `W` array such
...
pub enum XrefErrorKind {
    ...
    BadEntry,
    /// Total entries exceeded [`MAX_XREF_ENTRIES`] (pdfce guard).
    ...
    /// [`MAX_XREF_SECTIONS`] (cycle guard, pdfce policy).
...
///
/// pdfce's rule is to emit whatever the base file's **newest** section
...
struct WidthSpec {
    ...
    /// written in increasing field order, so trailing fields pdfce does
...
fn parse_xref_stream_section(
    ...
    // Field widths beyond the third are not pdfce's to reproduce (PDF
...
mod tests {
    ...
    // shapes that Pass A confirmed pdfce correctly fail-cleans on, so a
    ...
    fn generation_beyond_spec_max_is_rejected() {
        ...
        // spec-NON-conformant DATA in a structurally-valid record. pdfce
