...
//!
//! Before Pass 19.0, `pdfce-core` tracked the §9.3 text state **three
...
//! pdfce could not restore an ambient text rise it had never observed —
...
//!
//! pdfce scopes a text-state operator it emits for one run by **explicit
...
//!    re-normalized number is a diff in an object pdfce claims not to
...
//!    Emitting a guessed `0 Tc` would silently change content pdfce did
...
//!   by a content operator, and read by nothing in pdfce today.
//!
...
/// One of the six §9.3 text-state parameters pdfce models as a shared,
/// restorable quantity.
///
...
///
/// ```
/// use pdfce_core::text_state::TextStateParams;
///
...
/// pdfce's architecture and will grow as more of the object model becomes
...
pub enum AmbientRestoreError {
    ...
    #[error(
        "cannot restore the ambient {} ({param}) after this edit: {reason} — pdfce refuses rather \
...
impl AmbientValue {
    ...
    /// bytes, rather than pdfce's re-spelling of the same number.
    ///
    /// `false` only for [`AmbientOrigin::ObservedIndirect`] — the value is
    /// right, the spelling is pdfce's. Callers **disclose** that, exactly
    ...
    ///
    /// ```
    /// use pdfce_core::text_state::{AmbientValue, TextStateParam};
    ///
    ...
    pub fn restore_bytes(&self, param: TextStateParam) -> Result<Vec<u8>, AmbientRestoreError> {
        match &self.origin {
            ...
            AmbientOrigin::ObservedIndirect { .. } => {
                ...
                // number pdfce writes rather than like a `Display` of an
...
///
/// ```
/// use pdfce_core::text_state::{AmbientTextState, TextStateParam};
///
