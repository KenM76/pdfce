...
//! painted, and the same one `pdfce-render`'s annotation pass calls.
//!
...
//! its elements may nest. pdfce reports that tree ([`Layers::order`]) and
...
//! folder*, so the gap sits at a likely input. pdfce resolves it by
...
//!   pdfce still *lists* those groups, with [`Layer::in_order`] `false`,
...
//! and missed it.
//!
//! The standard states **no reader behaviour** for the violation. pdfce's
...
//! unregistered group, the implementation's does not, so pdfce reports it
//! `visible_by_default == true`.
//!
//! pdfce keeps the shared implementation's answer rather than correcting
...
//!   are scanned for the OCG references they name, never *applied* — pdfce
...
//!   resource `/Properties` dictionary; pdfce reads the *whole*
...
/// Maximum number of distinct layers reported (pdfce policy,
/// `ARCHITECTURE.md` §10).
///
/// §8.11 imposes no limit and Annex C's implementation limits do not
/// mention optional content, so this is pdfce policy on untrusted input,
...
pub struct Layer {
    ...
    /// "Untitled") because `CLAUDE.md` rule 4 forbids pdfce presenting
    ...
    /// amended in ISO 32000-2. Immaterial here — pdfce reads locks from
    ...
    /// the reader's user interface. pdfce reports them anyway, flagged,
...
pub struct Layers {
    ...
    /// `VisiblePages`, verbatim as a name. Reported, not acted on — pdfce
...
/// pdfce reports what it measured and leaves conformance judgements to a
...
pub struct LayerDiagnostics {
    ...
    /// and left the array alone — and not a conformance failure pdfce is
    ...
    /// accepted as a label rather than counted here. pdfce is lenient in
    ...
    /// ON"). pdfce follows the file rather than the *shall* — see the
...
///
/// Scanned for **discovery only**. pdfce never applies a usage
...
///    ordering is pdfce resolving ambiguity `DA-A3`. `[1 0 R [(Label)
...
mod tests {
    ...
    ///
    /// **Catches:** an unbounded recursive walk. `pdfce-core`'s panic-free
    ...
    /// `CLAUDE.md` rule 4: anything pdfce inferred is visible as an
