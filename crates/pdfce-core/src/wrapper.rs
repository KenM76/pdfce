...
//!
//! pdfce refuses an encrypted document by name: the parser sees `/Encrypt` in
...
//!
//! So without this module pdfce parses the file, renders the cover, reports
...
//! # How it is detected
//!
//! §7.6.7 wires a wrapper together with machinery pdfce already reads: the
...
//!
//! §7.6.7 is **ISO 32000-2 only**. ISO 32000-1 has no wrapper concept. pdfce
...
/// What pdfce found when it looked for a wrapper.
    ...
    pub fn message(&self) -> Option<String> {
        ...
        Some(format!(
            "What you can see is a COVER PAGE, not the document. This file is an \
             ...
             that pdfce cannot decrypt. Page counts, form fields and text extracted from \
...
mod tests {
    ...
    ///
    /// This is the whole point. pdfce's encryption refusal keys on
    /// `/Encrypt` in the trailer, and a wrapper has none — it is a plainly
    /// readable PDF. Without this detector pdfce reports success and shows
