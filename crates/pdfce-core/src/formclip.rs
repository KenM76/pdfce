...
//!
//! `pdfce-core` could have grown a `/DA` writer, a `/Q` writer, a `/DV`
...
//! pdfce deliberately does not execute (NF4, decision 008 §5.1). Expressing
...
//!   `/ParentTree` (§14.7.4.4). pdfce has no structure-tree writer, so
...
//!   about an object that did not exist when it was made. pdfce never reads a
...
//!   the moment the clip lands. pdfce still never *executes* it (NF4).
...
//!   re-sorting an array pdfce did not logically change.
...
/// first sixteen bytes of an unknown clipboard payload can tell the two pdfce
...
impl FieldClip {
    ...
    /// makes `/FT` required for a terminal field, and pdfce does not invent
    /// one.
    #[must_use]
    ...
    /// choose.
    ///
    /// pdfce never executes the script either way (decision 008 §5.1, NF4).
    #[must_use]
...
/// a closed set: pdfce chose every value, so it knows in advance what it might
/// have to say.
///
...
/// pdfce's guess, and these strings belong in a status line or a report.
#[derive(Debug, Clone, PartialEq, Eq)]
#[non_exhaustive]
...
mod tests {
    ...
    fn an_unrelated_payload_is_refused_by_the_magic_not_by_a_length_prefix() {
        ...
        assert_eq!(
            ...
            Err(ClipError::NotAClip),
            "the two pdfce clipboard formats must not be confusable",
        );
    }
