...
//! DISPLAY-ONLY (parse-nothing); pdfce sets `/R` and `/C` consistently. All
...
///
/// ```
/// use pdfce_core::dimension::{build_measure_dict, NumberFormat, Unit};
...
/// is free text a reader shows but never parses). pdfce writes a readable
/// `1 pt = <s> <unit>` form.
...
///
/// ```
/// use pdfce_core::dimension::build_ocg;
/// use pdfce_core::object::Object;
///
...
/// of pdfce-authored group OCGs, each with its default visibility.
///
...
///
/// ```
/// use pdfce_core::dimension::build_ocproperties;
...
pub fn build_ocproperties(group_ocgs: &[(ObjId, bool)], foreign_ocgs: &[ObjId]) -> Object {
    ...
    // Foreign OCGs first (preserved), then pdfce's own.
    ...
    config.insert(
        Name::from(b"Name"),
        Object::String(b"pdfce dimensions".to_vec()),
    );
