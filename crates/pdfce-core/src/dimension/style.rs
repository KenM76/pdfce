...
//!
//! ```text
//! 1. FACTORY DEFAULT   [`StyleDefaults::FACTORY`] — the values pdfce shipped
...
//!    inconsistency."* pdfce uses `Option::None`, everywhere, for every
//!    property.
...
//!    leading zeros from *the drafting standard* (§F.3). pdfce's cascade has
...
///
/// It is what pdfce has always drawn (`author::arrowhead` filled a triangle
...
pub enum ArrowForm {
    /// A solid filled triangle — ANSI/ASME mechanical practice, and pdfce's
...
impl ArrowForm {
    ...
    /// document carrying an arrow form a future pdfce added must degrade to
...
/// The bottom of the cascade: the values pdfce draws with when neither the ce
...
/// (`SolidWorks_Dimensions` §F.4). pdfce's cascade is a live link in both
...
pub enum StyleSource {
    /// pdfce's built-in default — neither the group nor the ce dimension set
    /// this property.
    Factory,
