...
//! number-format algorithm pdfce also encodes into the portable `/Measure`
//! dict (see [`super::measure_dict`]). Everything here is **pure** and has no
//! dependency outside `pdfce-core`; it is exhaustively unit-tested because it
...
//! dict's `/C` — the value pdfce stores AND the value the `/Measure` mirror
...
pub enum FractionMode {
    ...
    Decimal {
        /// Number of decimal places shown (fixed — pdfce shows the full
...
impl NumberFormat {
    ...
    ///
    /// This is the exact string pdfce's live readout and each baked `/AP`
    ...
    ///
    /// ```
    /// use pdfce_core::dimension::{NumberFormat, Unit};
    ///
...
/// Format a value to a fixed number of decimal places (no trimming — pdfce
...
///
/// ```
/// use pdfce_core::dimension::{format_measurement, NumberFormat, ScaleState, Unit};
///
...
///
/// ```
/// use pdfce_core::dimension::{preview_group_scale, ScaleEntry, Unit};
///
