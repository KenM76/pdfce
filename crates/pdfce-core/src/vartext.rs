...
//!   gives **no formula** — so [`auto_size`] is a *reviewable pdfce
...
//! therefore authors a minimal dict with **zero font bytes**, and pdfce's
//! own renderer paints it through its bundled Foxit substitute face
//! (`pdfce-render::font::select::by_name`). The `/Q` measurement uses the
...
//! but readers *shall* still honour the bare form (§9.6.2.1). pdfce
...
/// standard-14 face it denotes. For FreeText this is the single font pdfce
...
pub enum VarTextError {
    ...
    #[error(
        "the /DA font /{0} is a symbolic font; pdfce's variable-text generator is Base-14 Latin only"
    )]
    SymbolicFont(String),
}
...
/// glyphs touching the border. pdfce's own choice (the spec fixes none).
const TEXT_PAD: f64 = 2.0;

/// The line advance as a multiple of the font size (leading). 1.15 gives a
/// small amount of inter-line space; pdfce's own choice.
...
///
/// **This is pdfce's heuristic, not a spec formula.** §12.7.3.3 mandates
/// only that auto-size be "a function of the height of the annotation
/// rectangle" and gives no ratio, leading rule, or worked example. pdfce
...
///
/// ```
/// use pdfce_core::vartext::{default_appearance_string, TextColor};
///
...
/// The minimal, program-free standard-14 font dictionary (§9.6.2.1) pdfce
...
///
/// ```
/// use pdfce_core::vartext::standard14_font_dict;
...
///
/// ```
/// use pdfce_core::vartext::{parse_default_appearance, TextColor};
///
