...
//! pdfce never edited. That is a separate and larger decision, deliberately
...
mod tests {
    ...
    fn a_round_trip_returns_the_original_bytes() {
        let plain = b"pdfce brotli round trip, with enough text to actually compress. \
                      pdfce brotli round trip, with enough text to actually compress."
            .repeat(4);
    ...
    /// returns `Ok`, pdfce is silently rendering a prefix of a content stream
