...
//! consumers. pdfce's corrupted-stream regression tests (one per
...
//!
//! ## Resource ceiling (ARCHITECTURE.md §10.1 — pdfce policy, not spec)
//!
...
//! from [`FilterError::UnsupportedFilter`], which says "pdfce does not
...
///
/// pdfce policy (ARCHITECTURE.md §10.1): no Annex C limit exists for
...
pub enum FilterError {
    ...
    /// Decoded output crossed [`MAX_DECODED_LEN`] (pdfce guard) — the
    ...
    UnknownPngTag(u8),
    /// A standard filter pdfce doesn't implement yet (Pass 1 scope).
    ...
    /// that one means "pdfce cannot decode this at all", this one means
...
/// names because inline images legally use them; `pdfce_core::content`
...
mod tests {
    ...
    fn image_codec_in_the_byte_stream_cascade_is_its_own_error() {
        ...
        // way distinguishable from "pdfce cannot do this at all".
        for name in [
            &b"DCTDecode"[..],
            b"CCITTFaxDecode",
            b"JBIG2Decode",
