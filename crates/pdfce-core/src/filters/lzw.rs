...
//!
//! ## pdfce never WRITES LZW (rule R28)
//!
...
/// undefined by the spec; pdfce treats it as the default (1) rather
...
mod tests {
    ...
    ///
    /// **Test-only** (rule R28: pdfce ships no LZW encoder). `weezl`'s
    ...
    /// choice on pdfce's side, which is what these tests are for.
