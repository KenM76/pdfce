...
//! saving a form must never change a computed `/V`, or pdfce would be an
...
//! something pdfce **inferred**: it read a script it did not run and
...
//! state.
//!
//! # Evaluation order: `/CO` is normative, and pdfce follows it
//!
...
//!   dictionary".
//!
//! So pdfce evaluates in `/CO` order. That is both the conformant choice and
...
//! order pdfce preferred on its own reasoning would make pdfce disagree with
...
//! only one of the two is *pdfce's* mistake.
//!
//! # Where the standard stops, and pdfce has to choose
//!
...
//!
//! pdfce's choices, all disclosed rather than silent:
//!
//! | Situation | What pdfce does |
//! |---|---|
...
//!
//! A field pdfce could not compute confidently. Every such field appears in
...
pub enum OrderSource {
    ...
    /// dependencies. The file is non-conforming and pdfce chose an order the
...
impl OrderSource {
    /// Whether pdfce had to invent any part of the order.
    ///
    ...
    #[must_use]
    pub const fn is_pdfce_choice(self) -> bool {
...
pub enum Skip {
    /// pdfce declined to compute it — see [`Refusal`].
    Refused(Refusal),
    ...
    /// change.
    ///
    /// Reported rather than dropped: "pdfce checked this field and it was
...
pub struct RecomputePlan {
    ...
    /// Fields whose script pdfce recognises but cannot reproduce, or does not
    ...
    /// field carries a `/AA /C` — and that pdfce ordered those fields by a
...
/// A resolved evaluation order, and how much of it pdfce had to invent.
struct Ordering {
...
mod tests {
    ...
    enum Co {
        ...
        /// conforming shape.
        Listed,
        /// No `/CO` at all — non-conforming, and the case that forces pdfce
        /// to derive an order.
        Absent,
    }
    ...
    fn a_stale_total_is_planned_with_both_values() {
        ...
        assert!(!p.order_source.is_pdfce_choice());
        assert!(!p.is_empty());
    }
    ...
    /// order normative.
    ///
    /// pdfce could compute a "better" number here by ignoring `/CO`. It must
    /// not: posture B exists to reproduce what another reader produces, and a
    /// pdfce-only answer would make the same document show two different
    ...
    /// ★ **With no `/CO`, pdfce derives a dependency order — and says it
    /// did.**
    ///
    ...
    /// pdfce's own choice. It is a defensible one, and it is disclosed rather
    ...
    fn with_no_calc_order_a_dependency_order_is_derived_and_disclosed() {
        ...
        assert!(
            p.order_source.is_pdfce_choice(),
            "and the plan admits the order was pdfce's"
        );
    ...
    /// reads the file rather than a value pdfce declined to produce.
    #[test]
    fn a_refused_field_does_not_feed_the_cascade() {
        ...
        assert_eq!(
            uses.proposed, "5",
            "Bad's STORED 0 was used, not a value pdfce refused to compute"
        );
    }
    ...
    /// says pdfce did not run the script.
    #[test]
