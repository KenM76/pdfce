...
//! pdfce **never executes JavaScript** (standing rules R53–R57; decision 009
...
//! *read*, and pdfce adds two numbers.
//!
...
//! identity failure means *a function pdfce does not know* — refuse. An
...
//! in [`shape`]). Failing to recognise a real helper costs a recompute pdfce
...
//!   a computed `/V`, or pdfce would be an editor that silently rewrites
//!   documents it was only asked to look at.
//! - **The source script is left in place.** If pdfce's native recompute
//!   ever diverges from Acrobat's real semantics on an edge case, a
//!   downstream JavaScript-executing reader recomputes and *corrects* pdfce's
...
/// contradiction, and pdfce treats a contradiction as a reason not to act.
...
pub enum Trigger {
    ...
    Format,
    /// `/V` — validate. Advisory in pdfce; never enforced by execution.
    Validate,
    /// `/K` — keystroke. Advisory; pdfce's fills are operator-reviewed.
    Keystroke,
}
...
/// A helper pdfce **recognises and discloses but deliberately does not
...
/// though pdfce will not enforce it by execution. Classifying them as
/// `Custom` would throw that information away; reimplementing them would
/// enforce a rule pdfce cannot guarantee it evaluates the way Acrobat does,
...
pub enum AdvisoryHelper {
    /// An `AF*_Keystroke` input filter. Advisory: pdfce's fills are
...
/// What a `/JS` string is, as far as pdfce is willing to commit.
#[derive(Debug, Clone, PartialEq)]
...
impl ScriptClass {
    /// Whether pdfce can natively produce this helper's effect.
    ///
...
/// on `/C` was not generated by the Format tab, and whatever it is, pdfce
...
fn classify_calculate(call: &Call) -> ScriptClass {
    ...
    // Exactly two arguments. A third would mean a signature pdfce does not
    ...
    // list untrustworthy — pdfce would be summing a set it only partly
...
fn classify_advisory(call: &Call, trigger: Trigger) -> ScriptClass {
    ...
    // because its members share one purpose (filter input) and pdfce's
...
mod tests {
    ...
    /// crossed pair was not generated by Acrobat, and pdfce declines to
    /// guess what it was.
    #[test]
    ...
    /// `Custom` — pdfce does not fill in a missing argument with a default
    /// it invented.
    #[test]
