...
//! relies on.)
//!
//! Acrobat's behaviour was sourced through `pdfce-acrobat-librarian` into
...
//!   taken; no code was copied, and pdfce links nothing (`LEGAL.md` §6.1).
//! - Whether hidden or read-only fields participate: **unsourced.** No
//!   source checks a flag before including a field, so pdfce does not
...
//!
//! 1. **A name that resolves to no field**: pdfce **refuses the whole
...
//!    reports describe Acrobat throwing — so pdfce declines to pick, which
...
///
/// pdfce will not guess. The default treats a comma-bearing value as
...
pub enum Refusal {
    ...
    /// real-world reports describe an error — so pdfce refuses rather than
    ...
    /// the field would present pdfce's guess as the document's intent.
    NoOperands,
        ...
        match self {
            Self::UnresolvedOperand(name) => write!(
                ...
                "the script totals a field named {name:?} that this document \
                 ...
                 settled, so pdfce will not guess at the total"
            ),
...
/// it**, and each match contributes its own operand. That is pdfce's model
...
mod tests {
    ...
    ///
    /// The sources disagree about Acrobat here, so pdfce declines to pick.
    ...
    fn a_name_matching_several_representations_expands_in_place() {
        ...
        // multi-representation field takes in pdfce's flattened model.
