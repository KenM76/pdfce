//! What pdfce **says** about a script-driven field — decision 009 §7.
//!
...
//! 2. **whether pdfce ran it** — always: no, and
...
//!
//! Project rule 4, *fuzzy, never sneaky*: anything pdfce inferred is visible
//! before it becomes document state. A recomputed total is an inference —
//! pdfce read a script it did not run and reproduced what it believes the
...
//! pdfce's operator-facing text. The **machine-readable** counterpart is
...
/// Whether pdfce is able to reproduce a field's computation, and therefore
/// what it can honestly say about the value on screen.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Reproducibility {
    /// A whitelisted helper pdfce can recompute natively, and the operator
    /// has not asked it to. The stored value stands and **may be stale**.
    RecomputableNotRun,
    /// A whitelisted helper pdfce recomputed at the operator's request. The
    /// value on screen is pdfce's, and the source script remains in the file
    /// as the downstream authority.
    Recomputed,
    /// A script pdfce recognises but deliberately does not reimplement
    /// ([`AdvisoryHelper`]), or does not recognise at all. The stored value
    /// stands and may be stale; pdfce cannot offer to refresh it.
    NotReproducible,
}
...
pub struct Disclosure {
    ...
    /// Whether pdfce can reproduce it, and whether it has.
    pub reproducibility: Reproducibility,
    /// The value as it currently stands in the document, rendered for
    /// display. For [`Reproducibility::Recomputed`] this is pdfce's new
...
impl Disclosure {
    ...
    ///
    /// Every branch states that pdfce did not run the script. That is
    ...
    pub fn message(&self) -> String {
        match self.reproducibility {
            Reproducibility::Recomputed => {
                ...
                format!(
                    "{}: {} = {}{was}. Computed natively by pdfce from a recognised \
            ...
            Reproducibility::RecomputableNotRun => format!(
                "{}: computed by a recognised Acrobat built-in ({}) that pdfce does \
                 not execute — {}. Showing the stored value as last saved: {}. It may \
                 be stale if its inputs changed. pdfce can recompute this natively on \
                 request.",
                self.field, self.helper, self.computation, self.value
            ),
            Reproducibility::NotReproducible => format!(
                "{}: computed by a document script pdfce does not run — {}. Showing \
    ...
    match class {
        ...
        ScriptClass::Advisory(AdvisoryHelper::RangeValidate { lower, upper }) => {
            match (lower, upper) {
                ...
                // no bound would read as though pdfce had lost the bounds.
...
/// Build the disclosure for a field pdfce has just recomputed.
#[must_use]
...
mod tests {
    ...
    /// ★ **Every disclosure states that pdfce did not run the script.**
    ///
    ...
    /// that then reads as an authoritative pdfce computation.
    #[test]
    fn every_disclosure_says_pdfce_did_not_run_the_script() {
        let calc = sum_of(&["A", "B"]);
    ...
    /// A value pdfce did not compute is always marked as possibly stale.
    #[test]
    fn an_uncomputed_value_is_always_marked_possibly_stale() {
        for class in [
            ...
            assert!(
                ...
                "and the value must be attributed to the file, not to pdfce: {m}"
            );
        }
    }

    /// A recomputed value is NOT marked stale — it is pdfce's own — and it
    ...
    fn a_recomputed_value_is_not_stale_and_says_the_script_is_preserved() {
        ...
        assert!(!m.contains("may be stale"), "pdfce computed it: {m}");
    ...
    /// not reproducible — pdfce recognises it without offering to enforce it.
    #[test]
    ...
    /// A recognised-but-unrun helper advertises that pdfce could refresh it;
    ...
    fn only_a_reproducible_script_advertises_the_recompute() {
        ...
        assert!(
            ...
            "pdfce must not offer what it cannot do: {cannot}"
        );
    }
