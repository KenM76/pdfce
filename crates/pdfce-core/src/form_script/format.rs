...
//!
//! So this is a **pdfce invariant that deliberately declines spec-granted
//! latitude**, not a rule the spec supplies. The reasons are pdfce's own:
...
//! tables below come from `pdfce-acrobat-librarian`'s
...
//!   reimplementation — **behaviour only; no code was copied and pdfce links
...
//!
//! [`FormatOutcome`] has a decline arm, and it is used. A helper pdfce
...
//! see a formatted string and reasonably conclude pdfce had reproduced the
//! document's intent.
...
/// The result of asking pdfce to render a formatted display string.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum FormatOutcome {
    /// pdfce reproduced the helper's output.
    Rendered(Formatted),
    /// The helper is recognised, but a parameter's meaning is unsourced, so
    /// pdfce will not guess at the output.
    ///
...
impl FormatOutcome {
    ...
    /// itself concretely — "pdfce could not format this" tells an operator
        ...
        match self {
            ...
            Self::UnknownStyle { parameter, code } => Some(format!(
                "the script passes {parameter}={code}, a setting no available source \
                 documents; pdfce shows the stored value rather than guess at how \
                 Acrobat renders it"
            )),
            Self::NotNumeric => {
...
///
/// The consequence for pdfce's data model is worth stating plainly: a
...
/// The largest `nDec` pdfce will honour.
///
