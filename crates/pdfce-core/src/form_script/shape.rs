...
//!   `Custom` — costs the operator a recompute pdfce could have offered.
...
//! access, or an expression, its value depends on the runtime pdfce does not
...
//! | `AFSimple_Calculate("SUM", flds)` | `flds` is a runtime value; pdfce cannot know the operands. |
...
/// non-execution is not a restriction pdfce imposes on itself — it is simply
...
impl fmt::Display for Call {
    ...
    /// `--json` output. Not a round-trip of the source: it prints what pdfce
...
impl Parser<'_> {
    ...
    /// field name that pdfce decoded differently from Acrobat would look up
...
mod tests {
    ...
    /// The `Display` form shows what pdfce understood, which is what a
