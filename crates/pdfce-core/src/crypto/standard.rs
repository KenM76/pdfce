...
pub enum EncryptionUnsupported {
    ...
    /// deleted because Table 25's `/CFM` set is closed and pdfce now implements
    ...
    #[error(
        "{0} encryption is not implemented yet (pdfce reads RC4 40-128 bit, AES-128 and AES-256 at /R 5)"
    )]
...
impl EncryptionConfig {
    ...
    /// pdfce could not apply the password preprocessing the revision asks for.
    ///
    /// `true` only for a `/R` 5 document whose supplied password contains a
    /// byte RFC 4013's SASLprep step could have changed — see
    /// [`PreparedPassword`]'s docs for why pdfce attempts such a password
    ...
    /// one.
    ///
    /// **The result is a report.** pdfce never prefers the encrypted copy over
...
mod tests {
    ...
    fn r5_encrypt(overrides: Vec<(&str, Object)>) -> Dict {
        ...
        // /Length 256, the value 2.0-as-printed asks for and the value pdfce's
