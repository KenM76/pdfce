...
//!
//! `pdfce-render` parses a donor font file and subsets it, producing a
...
//!
//! Subsetting is a write concern, so `pdfce-core` looks like its natural
...
//! `pdfce-render`. Putting a subsetter here would add a second font parser
...
//! core emits, and `pdfce-core` gains no new dependency.
//!
...
/// here because they come from the donor's own tables, and `pdfce-core` has
...
/// Everything `pdfce-core` needs to emit an embedded subset, and nothing
/// about how it was produced.
///
/// Deliberately free of any font-crate type. The seam is plain data so that
/// `pdfce-core` never acquires a font parser (R21 / decision 021 §3.2), and
...
pub enum FontEmbedError {
    ...
    #[error(
        "this font's outlines are {kind:?}, and pdfce can currently embed only TrueType (glyf) \
...
pub fn build_objects(
    ...
    // `clippy::indexing_slicing` is DENIED crate-wide because pdfce-core eats
...
fn widths_array(glyphs: &[SubsetGlyph]) -> Vec<Object> {
    ...
    // and while `glyphs` here is pdfce's own construction, carving out an
...
mod tests {
        ...
        for id in &written {
            assert!(
                ...
                "R107 VIOLATION: font embedding wrote to {id:?}, which already exists in the \
                 ...
                 object breaks the round-trip invariant (ARCHITECTURE.md §5) for a file pdfce \
        ...
        for bad in ["abcdef", "ABCDE", "ABCDEFG", "ABC1EF", ""] {
            ...
            assert!(
                ...
                "subset tag {bad:?} should have been refused — §9.6.4 requires exactly six \
                 ...
                 a subset to every consumer, including pdfce's own is_subset_tag"
            );
        }
    ...
    /// what standing rule R110 turns on. pdfce-authored CMaps are injective
