...
//! project.
//!
//! ## Why this lives in `pdfce-core` and not in a shell
//!
//! It looks like presentation data, and it is not. `pdfce-gui`'s page-size
//! chooser, `pdfce-cli`'s `set-page-size --size`, and any future web shell
...
//! `pdfce-core` may not depend on a GUI crate; it says nothing against
...
//! conversion at full precision. Doing the division here means pdfce's A1
...
//!   at all: the corpus pdfce is measured against is drawing sheets.
//!
//! # Failure modes
//!
