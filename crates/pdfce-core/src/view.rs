...
//!    [`ByteSpan`] covers". Every stream in pdfce is span-backed rather
...
/// to actual bytes.
///
/// Two variants, because pdfce has exactly two kinds of readable document
...
/// use pdfce_core::view::StreamSource;
///
...
/// file as the operator currently has it). Every read path in pdfce that
...
/// this: 45 of `pdfce-render`'s 50 `Document` call sites compiled
...
impl<'a> DocumentView<'a> {
    ...
    /// its original signature so `pageops`, `pdfce-cli` and the page-op
