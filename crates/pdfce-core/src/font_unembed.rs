...
//! deriving one of its own**. There is exactly one classifier in pdfce, so
...
//! research — no source found either way — so this is pdfce's own decision,
...
//! the `+` all mean "no tag, this is the name"), and pdfce mangling a font
//! genuinely called `AB+Condensed` would be pdfce inventing data.
//!
//! The policy is overridable ([`SubsetTagPolicy::Keep`]) because a caller
//! comparing pdfce's output against another tool's byte-for-byte has a
...
//! a whole-document reference count, which pdfce does not have; the bound
...
/// *claim the file makes about itself*; pdfce does not check conformance
...
/// (`optimize__font_unembedding.md`), so pdfce is not matching a behaviour
...
///
/// pdfce has no XMP parser and this is not the Pass to acquire one. The
...
pub enum SubsetTagPolicy {
    ...
    ///
    /// For a caller comparing pdfce's output against another tool
...
pub enum UnembedBlocker {
    ...
    /// The `/FontDescriptor` is a **direct** dictionary somewhere pdfce
...
mod tests {
    ...
    /// freeing it would blank the font pdfce refused to touch.
    #[test]
