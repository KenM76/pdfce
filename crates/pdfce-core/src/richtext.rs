...
//! value because one attribute is unfamiliar would make pdfce unable to
//! read files Acrobat reads fine.
//!
//! This is a deliberate asymmetry with pdfce's usual strictness. It is
...
//! `/NeedAppearances`).
//!
//! So any pdfce rendering built on these runs is **policy, not
//! conformance** — a defensible choice among several, not "what the
//! standard says". Project rule 4 requires that be disclosed as pdfce's
...
///
/// ```
/// use pdfce_core::richtext::{self, Align};
///
...
/// unit is points, and rejecting that would lose a size pdfce could
