//! # The Document — pdfce's single object model (load side)
//!
...
//!   pdfce policy — `5 0 obj 5 0 R endobj` is legal syntax and must
//!   not loop).
...
///
/// pdfce policy (ARCHITECTURE.md §10): the spec permits
...
impl Document {
    ...
    /// silence — which is exactly why pdfce imposes it.
    #[must_use]
    ...
    /// and pdfce rebuilt it by scanning the file for `N G obj` headers
    ...
    /// (F.1: an incremental update *"shall"* de-linearize). pdfce never
    ...
    ///
    /// This is pdfce's **default** save mode (`ARCHITECTURE.md` §5):
    ...
    /// never something pdfce decides silently (R36, decision 007 W7).
    ///
    /// # Errors
    ///
    ...
    ///    pdfce has no deletion path to have created those entries in
