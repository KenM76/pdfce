...
//! pdfce's posture at *this* layer is simpler and stricter — there is no
...
//! rather than to one of pdfce's own: a file that lies about its
...
//!   rather than redeclared, so pdfce has one answer to "how big may a
...
///
/// A pdfce guard, not a spec limit — §7.11 puts no bound on the size of
...
pub struct Attachment {
    ...
    ///
    /// pdfce reports it **raw** anyway, and that is a deliberate design
    ...
    /// What pdfce could cheaply determine about [`Attachment::declared_size`].
    ...
    /// lifetimes. pdfce reports both entries — collapsing them would hide
...
/// rule is about pdfce not quietly deciding things, and quietly dropping a
...
/// payload uses a compression pdfce cannot decode".
...
pub struct ExtractedAttachment {
    /// The decoded payload. Opaque bytes — pdfce does not interpret,
...
/// to be sorted, and pdfce walks `/Kids` in array order rather than
...
/// usually does, because "no attachments" and "attachments pdfce could not
/// read" look identical here.
///
/// # Examples
///
/// ```
/// use pdfce_core::attachments::{list_attachments, AttachmentKind};
...
///
/// ```
/// use pdfce_core::attachments::list_attachments_with_notes;
/// use pdfce_core::document::Document;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// let doc = Document::from_bytes(
...
/// enumerates everything; honouring it could only ever cause pdfce to skip
...
///
/// ```
/// use pdfce_core::attachments::{attachment_bytes, list_attachments};
/// use pdfce_core::document::Document;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// let doc = Document::from_bytes(
...
/// return **that** document's bytes. pdfce cannot detect the confusion
...
///
/// They came from a file inside a file. pdfce does not execute, open, or
...
///
/// ```
/// use pdfce_core::attachments::{extract_attachment, list_attachments, DeclaredSizeCheck};
/// use pdfce_core::document::Document;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// let doc = Document::from_bytes(
...
///
/// // The document claimed 999999 bytes and delivered ten. pdfce says so.
/// assert_eq!(
///     got.size_check,
...
/// 4) — "pdfce renamed this file" with no reason is exactly the sneaky
...
pub enum NameHazard {
    ...
    /// it came from **pdfce's own §7.9.2 decode failing**
    ...
    /// a name pdfce could not decode is one the operator should be told
...
///
/// # Why pdfce exposes the raw name AND a sanitiser, rather than only one
///
...
/// `..\..\Windows\System32\evil.exe`, then pdfce would be *lying about the
/// document* — and the operator investigating a suspicious file would be
/// looking at pdfce's cleaned-up version of the evidence, with no way to
...
/// what pdfce would write to disk, *plus* a [`SafeName::hazards`] list
/// explaining every difference. That satisfies project rule 4 exactly as
/// written — the correction pdfce made is visible before it becomes state,
...
/// pdfce is a portable application and its output should not depend on
...
/// at all (module docs: pdfce never runs an attachment).
///
/// # Examples
///
/// ```
/// use pdfce_core::attachments::{sanitize_attachment_name, NameHazard};
///
...
mod tests {
    ...
    /// disk has a different name than the one pdfce displayed.
    #[test]
