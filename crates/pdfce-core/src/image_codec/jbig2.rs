...
/// # Errors
///
/// [`ImageCodecError::TooLarge`] when the page geometry crosses a pdfce
...
pub(super) fn decode(
    ...
    // CCITT adapter: an overflow means pdfce stopped accepting rows, so
    ...
    Ok(CodedImage {
        ...
        // than the declared one is what lets `pdfce-render` mark a short
...
/// Wrap a `hayro-jbig2` failure as a structured pdfce error.
///
...
/// unsupported *frame types* are a fixed, enumerable set that pdfce
...
/// counted and reported, never acted on: `pdfce-render` keeps the
...
/// The vendor sink impl: T.88's `1 = black` becomes pdfce's
/// `1 = white` (module docs).
