...
//!
//! The second tier of pdfce's two-tier filter architecture
...
//! `pdfce-render` implements the per-filter split: the dictionary's
...
//!
//! `pdfce-core` hands `pdfce-render` the codec's own samples and its
//! declared colour model. Only `pdfce-render` applies `/Decode`
...
//!
//! ## Resource ceilings are pdfce's, never the vendor's (rule R25)
//!
...
//! ceiling below is therefore set explicitly by pdfce and documented
//! with its reasoning.
...
/// `pdfce_render::image::MAX_IMAGE_PIXELS` re-exports it so the codec
...
/// component count pdfce maps (CMYK). Checked against the decoder's own
...
///
/// Mapping this onto `/ColorSpace` is `pdfce-render`'s job (rule R26).
...
pub enum CodecColorModel {
    ...
    /// `pdfce-render`). For YCCK storage the mandated §13.1 inverse
    ...
    Unspecified,
    /// The codestream declared a component count pdfce has no PDF
...
pub struct CodecNotes {
    ...
    /// carry no residual polarity ambiguity, and pdfce's output
    ...
    /// un-inverted and share the gap; pdfce's differentiator is that
...
/// (rule R26). That is `pdfce-render`'s job.
...
pub struct CodedImage {
    ...
    /// byte-stream filters produce, so `pdfce-render`'s existing
...
pub enum ImageCodecError {
    ...
    /// The codestream's declared geometry crosses a pdfce ceiling
    /// ([`MAX_IMAGE_PIXELS`], [`MAX_IMAGE_DIMENSION`],
    /// [`MAX_IMAGE_SAMPLE_BYTES`]).
    #[error("image exceeds pdfce's decode ceilings ({MAX_IMAGE_PIXELS} pixels max)")]
    TooLarge,
...
///
/// ```
/// use pdfce_core::image_codec::{terminal_codec, Codec};
...
pub fn decode_image_view_with(
    ...
    // is not a chain pdfce can make sense of — see `CodecNotTerminal`.
...
mod tests {
    ...
    fn cmyk_jpeg_passes_four_raw_components_through_and_is_counted() {
        ...
        // stored. pdfce applies NO "Adobe inversion" of its own (R29 —
    ...
    fn cmyk_jpeg_with_a_decode_array_is_not_polarity_unverifiable() {
        ...
        // pdfce-render's job, so the samples must be byte-identical to
    ...
    fn jpx_decodes_a_raw_codestream_identically_to_its_jp2_container() {
        ...
        // what proves pdfce reads geometry and colour from the
    ...
    fn jpx_dictionary_geometry_disagreement_is_counted_and_the_codestream_reported() {
        ...
        // but gives no conflict rule. pdfce reports the codestream's
    ...
    fn jpx_ignores_the_decode_array_entirely_at_this_layer() {
        ...
        // `pdfce-render` is where the suppression is enforced for real.
    ...
    fn jpx_unsupported_enumerated_colour_space_is_a_named_diagnostic() {
        ...
        // exists — so pdfce names the gap rather than reporting a
    ...
    fn jpx_refuses_a_geometry_past_the_pdfce_ceiling_before_decoding() {
        ...
        // bound the PRODUCT — so pdfce's ceiling is the only thing
    ...
    #[test]
    fn ccitt_refuses_a_geometry_past_the_pdfce_ceiling() {
        // Rule R25: the ceiling is pdfce's, and it is checked on the
    ...
    fn ccitt_corrupt_data_errs_and_never_returns_a_short_page() {
        ...
        // some rows may already have been written when it errors; pdfce
        ...
        // meaningless) picture. "Fail-clean" means pdfce never *invents*
        // samples; it cannot mean pdfce detects a codec's own
