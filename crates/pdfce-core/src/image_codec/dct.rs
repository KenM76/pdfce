...
//! glue is where pdfce's bugs would live (decision 005 §6.5) — so the
...
//! 3. **Impose pdfce's own resource ceilings** (rule R25), never the
//!    vendor defaults.
...
//! unconditionally** (pdfce must never let `/DecodeParms` override an
...
//! | 4 | 1 or 2 | passthrough, then **pdfce's own** YCC→CMY inverse | `Cmyk` |
...
//! convention. Neither is what §7.4.8 asks for, so pdfce requests a raw
...
//! 3-component transform-0 image at the moment pdfce has to choose an
...
//! pdfce's side so the request is right the first time.
//!
...
//! null rule, now standing rule **R29**: pdfce **never** applies an
...
//! applied by `pdfce-render` (§8.9.5.2) — `/Decode [1 0 1 0 1 0 1 0]`
...
//! and no further polarity step exists to take. Empirically, pdfce
...
/// is about the number being one pdfce chose and can be seen to have
...
/// byte outside 0–2; [`ImageCodecError::TooLarge`] when a pdfce ceiling
...
pub(super) fn decode(
    ...
    // --- pdfce's ceilings, set explicitly (rule R25) ------------------
    ...
    // remains pdfce-render's job alone.
...
/// What pdfce asks `zune-jpeg` to hand back, and what that means.
///
...
fn route(frame: &Frame, transform: u8) -> Result<(ColorSpace, CodecColorModel), ImageCodecError> {
    ...
    // parsing; pdfce pre-sniffs precisely so it becomes a named,
...
/// The `ColorSpace` to request when pdfce wants the codestream's own
/// samples untouched.
///
...
///   pdfce already knows the component count from its own SOF sniff, so
...
/// Component count implied by the colourspace pdfce requested.
...
/// Wrap a `zune-jpeg` failure as a structured pdfce error.
///
...
/// A corrupt-codestream error raised by pdfce's own marker walk.
