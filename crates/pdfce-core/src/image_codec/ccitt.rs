...
//! geometry reconciliation — which is precisely where pdfce's own bugs
...
//! left the trichotomy, the byte alignment and the polarity as pdfce
...
//!    flag, already XORed with its `invert_black` setting. pdfce's
...
//! decode **zero rows**. pdfce therefore supplies a bound, in the order
...
//! 3. otherwise [`row_ceiling`] — a pdfce-derived bound, never an
...
//!
//! ## Resource ceilings are pdfce's (rule R25)
//!
...
impl Default for Params {
    ...
    /// are what pdfce applies when a key is **absent**, regardless of
...
/// # Errors
///
/// [`ImageCodecError::TooLarge`] when `Columns`/`Rows` cross a pdfce
...
/// asked for damage tolerance pdfce does not implement (rule R27);
...
pub(super) fn decode(
    ...
    // still producing rows when pdfce stopped accepting them, so any
...
/// The row count Table 11 lets pdfce know in advance, if any.
///
...
/// The largest row count pdfce will decode for a given width.
///
...
/// Map a `hayro-ccitt` failure to a structured pdfce error.
///
...
/// `hayro-ccitt` does not implement and pdfce does not add. When a file
...
/// already have been written when it errors; pdfce deliberately does
...
/// A corrupt-codestream error raised by pdfce's own parameter checks.
...
/// exactly pdfce's notion of "white" and needs no further translation.
