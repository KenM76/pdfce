...
//! of pdfce's chosen decoders — `hayro-ccitt` and `hayro-jbig2` — hand
...
//! `fax`: the sink is where pdfce's **own** row stride, byte padding and
...
//! pdfce reports `width = columns`, *not* the rounded-up width: the
//! rounding is a property of the byte layout, not of the picture, and
//! `pdfce-render` derives the identical stride from `columns` with its
...
//! the entire responsibility on pdfce. Once the budget is crossed the
...
impl BilevelSink {
    ...
    /// leaves them unspecified and `pdfce-render` reads exactly
