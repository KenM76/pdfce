...
//! `moxcms`, neither of which belongs in `pdfce-core`). It is the only
...
//! and any embedded ICC profile. `pdfce-render` owns the other half —
//! choosing between dictionary and codestream, and suppressing
//! `/Decode`.
//!
//! ## Bit depth: pdfce delivers 8, range-scaled — a decision, documented
//!
...
//! §8.9.3's sample layout — which `pdfce-render`'s unpacking path
...
//! pdfce determines **8**, and scales each component from its own
//! declared depth:
//!
//! ```text
...
//! legally declare as large as 128 — a shift overflow. pdfce's own loop
...
//! interaction a GAP (clause 11 not yet ingested). So pdfce:
//!
...
//! validation rule about the image dictionary, and `pdfce-render` is
...
//! read earns its place. pdfce applies no colour management: the
...
//! colour space is not one pdfce can approximate becomes
...
/// Maximum number of T.800 tiles pdfce will decode in one image.
///
...
/// 1024 × 1024, which is 30 tiles at pdfce's 32 Mpx ceiling; the most
...
///
/// Like every ceiling in this module it is pdfce's own number, not a
...
/// Highest per-component bit depth pdfce will scale from.
///
...
///   `"JPX/unsupported-marker"`) and a component bit depth pdfce will
...
pub(super) fn decode(
    ...
    // reporting the emitted count is what lets `pdfce-render` mark a
    ...
    Ok(CodedImage {
        ...
        // The depth pdfce DELIVERS, which Table 89 makes the conforming
...
/// Map the vendor's colour space onto pdfce's codec-level colour model,
...
/// pdfce does not otherwise interpret ICC profiles, and this is used
...
/// `pdfce-render` keeps the dictionary's numbers for placement and the
...
/// and is not the 8 pdfce delivers — including the common, superficially
...
/// layer does not have and should not acquire — `pdfce-render` already
...
/// Wrap a `hayro-jpeg2000` failure as a structured pdfce error, naming
...
fn map_error(err: hayro_jpeg2000::DecodeError, data: &[u8]) -> ImageCodecError {
    ...
    match err {
        ...
        // and draw the image with (wrong) device colours. pdfce cannot
...
/// A corrupt-stream error raised by pdfce's own checks.
