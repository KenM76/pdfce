//! # pdfce-core — the GUI-agnostic PDF engine
//!
//! This crate is the heart of pdfce (docs/ARCHITECTURE.md §3, §4). Its
...
//! separate `pdfce-render` crate).
//!
//! ## Load-bearing invariant
//!
//! `pdfce-core` **must not** depend on any GUI/windowing crate
...
//! greps `cargo tree -p pdfce-core` to enforce it. The only dependency at
...
//! flow and `pdfce-cli inspect`) without yet standing up the tokenizer or
...
// pdfce-core parses untrusted input, so a panic reachable from library code is
...
/// being a breaking change for downstream consumers such as `pdfce-cli`.
...
///
/// ```
/// use pdfce_core::{probe_header, PdfVersion};
///
