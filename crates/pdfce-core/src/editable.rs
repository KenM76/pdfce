...
//! would mean writing a second parser to read it back, and pdfce would then have
...
//!
//! pdfce has had incremental save as its **default** save mode from the start
...
//! breaks signatures because of *how it writes*; pdfce breaks one only because
//! of *what you changed*.
//!
...
pub fn export(doc: &Document) -> Result<Vec<u8>, EditableError> {
    ...
    out.extend_from_slice(
        b"% Editable export produced by pdfce. Object streams expanded, stream\n\
          ...
          % `pdfce-cli import-structure` to append only what you changed as an\n\
