...
//! re-deriving one**. There is exactly one font classifier in pdfce, so the
...
//! the number of fonts pdfce found something to do with.
//!
//! # ★ The crate boundary: the shell resolves a NAME, core takes BYTES
//!
//! `pdfce-render::FontEnvironment` — which knows what faces exist on disk —
//! lives in `pdfce-render`, and `pdfce-core` must never depend on it
...
//!   had none — **from pdfce's own compiled Adobe Core-14 AFM tables**
...
//!
//! So pdfce writes them, from data it already compiles in for text
...
//! [`crate::text_extract::font::ExtractFont`] — the one resolver in pdfce
...
//! encoding. The moment pdfce embeds a program, that clause starts pointing
//! at **the program pdfce chose**, whose internal encoding is a property of
//! a face the document never named.
//!
//! Leaving `/Encoding` absent would therefore make the document's *text*
//! depend on pdfce's donor choice. Writing the encoding out — from pdfce's
...
//!
//! pdfce resolves that by re-declaring the font dictionary as `/TrueType`,
...
//! 3. The descriptor pdfce writes carries the Table 123 `Nonsymbolic` flag,
//!    which is what puts the reader on Branch A in the first place.
//!
//! For a bare-CFF donor (including pdfce's own bundled Base-14 faces) none
...
//! character collection the donor must implement, and pdfce cannot verify
...
//! *"should not be incorporated into a PDF file."* pdfce reads the OpenType
...
//! prohibition comes from the font's licence, not from ISO 32000-1. pdfce
...
//! embed a font the operator did not select, with a face pdfce chose for a
...
//! context, and nothing here claims conformance: pdfce does not validate
...
/// The largest donor program pdfce will embed, in bytes.
///
...
///
/// **This is the inference rule 4 governs.** pdfce is choosing a font
...
/// applying a well-known family equivalence, or falling back to a face pdfce
...
pub enum FontMatch {
    ...
    Alias,
    /// The donor is one of the faces pdfce itself ships as a standard-14
...
/// seam is plain data, so `pdfce-core` acquires neither a font parser nor an
...
pub struct SuppliedFont {
    ...
    /// unmodified**. pdfce does not subset, re-order tables, or strip
...
pub enum ProgramFormat {
    ...
    OpenTypeCff,
    /// A bare CFF program (`Type1C` / `CIDFontType0C` framing). pdfce's own
    ...
    Collection,
    /// The bytes match no font framing pdfce recognises.
    Unrecognised,
}
...
impl ProgramFormat {
    ...
    /// `pdfce-core` must not gain a font-parsing dependency, and thirty
    ...
    ///
    /// ```
    /// use pdfce_core::font_embed_missing::ProgramFormat;
    ///
...
pub enum EmbedShape {
    ...
    /// metrics: pdfce writes `/FirstChar`, `/LastChar`, `/Widths`, a
...
pub enum EmbedBlocker {
    ...
    /// would be a repair rather than an embed, and pdfce does not silently
    ...
    Type3,
    /// A simple font with no `/Widths` that is not one of the standard 14,
    /// so pdfce has no source for the advances the file assumes.
    NoMetricSource,
    /// The donor bytes match no font framing pdfce recognises.
    ProgramUnrecognised,
    /// The donor is a font **collection** (`ttcf`). A collection holds
    /// several faces; the PDF names one font, and picking a face would be
    /// pdfce guessing which.
    ProgramIsCollection,
    ...
    /// A **symbolic** font offered a donor pdfce inferred rather than one
    ...
    pub const fn reason(&self) -> &'static str {
        match self {
            ...
            Self::ProgramDeclaredButUnreadable => {
                "This font says it carries a program, but those bytes could not be read. pdfce \
                 will not overwrite a program it failed to parse — that would be repairing the \
                 file, not embedding a font."
            }
            Self::NoSourceFont => {
                "No font file for this face was found. Point pdfce at a folder containing it — \
            ...
            Self::NoMetricSource => {
                "This font carries no character-width table and is not one of the 14 fonts every \
                 reader knows, so pdfce has no record of the spacing this document assumes. \
                 Embedding a font here would move the text on the page."
            }
            Self::ProgramUnrecognised => {
                "The file offered for this font is not in a format pdfce recognises as a font \
            ...
            Self::EmbeddingForbidden { .. } => {
                "The font file offered here says in its own licensing field that it may not be \
                 embedded in a document. pdfce will not embed it. Use a face whose licence \
                 permits embedding."
            }
            ...
            Self::ProgramTooLarge { .. } => {
                "The font file offered here is larger than pdfce will embed. Supply a smaller \
                 face."
            }
        }
    }
...
pub struct EmbedPlan {
    ...
    /// conformance, and pdfce validates none of it.
    pub pdfa: PdfaClaim,
...
/// case-folding `Arial` onto `ARIAL` would be pdfce deciding two different
/// names are one.
...
fn classify(
    ...
    match format {
        ...
        ProgramFormat::Type1 => {
            ...
            // `eexec` boundary and the fixed-content tail. pdfce does not
    ...
    //
    // So the guard is written against the mapping pdfce is about to produce,
    ...
    //   or pdfce is writing one out of its compiled Annex D.5/D.6 tables.
    //
    ...
    // dictionaries with no `/Encoding` — precisely the case pdfce pins from
...
/// inside [`EmbedShape::Synthesise`], where pdfce is already authoring the
/// descriptor, the widths and the encoding from its own data — so the
/// re-declared font is one pdfce fully describes rather than one it has
...
/// pdfce of §9.6.6's base-encoding-plus-`/Differences` chain, including the
...
/// Build the `/FontDescriptor` for a standard-14 face pdfce is describing
...
mod tests {
    ...
    ///   pdfce writes the Annex D.6 name table itself and glyph selection
    ...
    ///
    /// The second case uses a stub bare-CFF donor: `pdfce-core` never parses
    ...
    fn the_symbolic_guard_fires_on_cmap_mapping_and_stands_aside_for_name_mapping() {
        ...
        // -- allowed: pdfce writes the name table, so selection is by name.
        ...
        assert!(!t.redeclared_truetype);
        assert!(t.encoding_written, "the Annex D.6 name table is pdfce's");
    ...
    /// objects, not on the plan, because the plan is what pdfce *intended*.
    #[test]
