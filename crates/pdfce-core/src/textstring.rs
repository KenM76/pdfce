...
//!
//! pdfce's policy for an undefined code is [`DecodedText::exact`] going
...
//! the way pdfce would have written it" helper: `ARCHITECTURE.md` §5
//! requires that a text string pdfce did not logically modify is
...
///
/// ```
/// use pdfce_core::textstring::pdf_doc_char;
///
...
///   no recovery rule, so U+FFFD is disclosed pdfce policy);
...
///
/// ```
/// use pdfce_core::textstring::{decode_text_string, TextStringForm};
///
...
///
/// ```
/// use pdfce_core::textstring::decode_utf16be_bytes;
///
...
///
/// **Only for values the operator actually changed.** A string pdfce did
...
///
/// ```
/// use pdfce_core::textstring::{decode_text_string, encode_text_string};
///
