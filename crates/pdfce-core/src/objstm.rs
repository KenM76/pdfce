...
//! pdfce therefore does not follow it — which also means the "damaged
...
///
/// pdfce policy (ARCHITECTURE.md §10.1): ISO 32000-1 sets no limit —
...
pub enum ObjStmError {
    ...
    /// the loader that is building the document — pdfce refuses rather
    ...
    BadFirst,
    /// `/N` exceeded [`MAX_OBJSTM_OBJECTS`] (pdfce guard).
    ...
    /// ambiguity; the spec does not define reader behaviour, so pdfce
