...
//! pdfce carries the link, breaks the destination, and **says so**.
//!
...
//!
//! `crate::writer`'s R39 discipline says pdfce never *synthesises* an
...
/// tree (pdfce policy, `ARCHITECTURE.md` §10).
///
...
/// Maximum objects one assemble may copy (pdfce policy).
///
...
/// combine is a **confirmed documented default**. pdfce therefore adopts
...
pub struct AssembleOptions {
    ...
    ///   recommends pdfce *"leave `/PageLabels` numerically stale exactly
    ///   as Acrobat does"* for this Pass. pdfce matches that **and emits
...
pub struct AssembleReport {
    ...
    ///
    /// pdfce declines to copy half a field: §12.7.3.1 identifies a field
    ...
    ///
    /// pdfce does not carry the `/Dests` name tree: a name tree merged
    ...
    /// the insert case.
    ///
    /// Acrobat leaves it stale silently; pdfce leaves it stale and says
...
/// | `/Perms`, `/Encrypt` | **no** | signature permissions and encryption do not survive into a new document, and carrying either would be a claim pdfce cannot honour |
...
enum WidgetCoverage {
    ...
    None,
    /// Some are and some are not — the case pdfce refuses.
    Partial,
...
impl Copier {
    ...
    fn finish(mut self, catalog_num: u32, version: PdfVersion) -> Vec<u8> {
        ...
        let permanent = fileid::changing_identifier(b"pdfce/new-document/permanent", 0, body);
        let changing = fileid::changing_identifier(b"pdfce/new-document/changing", 0, body);
        trailer.insert(
            Name::from(b"ID"),
            Object::Array(vec![
