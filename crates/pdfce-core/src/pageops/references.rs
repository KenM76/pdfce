...
//!   pdfce discloses them (`core_ops__delete_pages.md` recommends pdfce
...
//! pretending to analyse that would be a claim pdfce cannot support.
...
/// (pdfce policy, `ARCHITECTURE.md` §10 — untrusted input, unbounded in
/// the spec).
pub const MAX_NAME_TREE_NODES: usize = 100_000;

/// Maximum name-tree depth (pdfce policy). §7.9.6 balances these trees;
/// anything this deep is damage or hostility.
pub const MAX_NAME_TREE_DEPTH: usize = 64;

/// Maximum outline items walked (pdfce policy). Bounds both the
...
/// and recommends pdfce surface these instead of leaving them silently
