...
//! 2. **Decode** — through [`crate::image_codec`], so every codec pdfce can
//!    read (Flate/LZW/RunLength raw samples, DCT, CCITT, JBIG2, JPX) is an
//!    image pdfce can redact. A codec pdfce cannot decode is a placement it
//!    cannot destroy, and that is reported **by name**, per placement — never
//!    masked, never silently skipped (see "What happens to a placement pdfce
...
//! this shape, and it is the common case the pdfceGUI request of 2026-09-03
//! asked for by name.
//!
...
//!
//! ## What happens to a placement pdfce cannot destroy
//!
//! A placement is undestroyable when its image cannot be decoded (a codec
//! feature pdfce has not implemented, a corrupt codestream, a bit depth
...
/// invisible, so nothing is lost by clearing it, and a matrix pdfce cannot
