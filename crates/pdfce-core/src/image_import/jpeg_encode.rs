...
//!
//! `ROADMAP.md` **R28** — *"Read-compat only: pdfce writes none of these
//! codecs. No image encoder enters any pdfce crate without a new decision
...
//! permissive ones. IJG imposes no source-disclosure obligation (so pdfce's
...
//! cannot be lost to a dependency-set change. See `crates/pdfce-core/Cargo.toml`
...
//! cannot tell. R29 is the standing rule that came out of it: *"pdfce never
...
//!
//! So pdfce feeds the encoder the **complement of its true-ink CMYK
...
//! passed through. The complement is not a polarity flip pdfce invented — it
//! is §13.1's own `255 −`, which `jpeg-encoder` parameterises onto the `K`
//! input instead of the C/M/Y inputs. Decoding it with pdfce's
...
//!
//! 1. **No `/Decode` array is needed or written.** R29 stays intact: pdfce
...
//!    conformance corpus have, and the one pdfce's render was verified
...
//!    would have been byte-correct *and* would have made pdfce emit the exact
...
//!
//! `crates/pdfce-core/tests/image_placement.rs` asserts this in **sample
...
//! | Refusal | Why |
//! |---|---|
//! | quality outside 1–100 ([`ImageImportError::InvalidQuality`](super::ImageImportError::InvalidQuality)) | A clamp would be pdfce choosing an encoder setting the operator did not choose, and then storing the result permanently. Rule 4 wants inferences *disclosed*; the cheaper honest answer for a plainly out-of-range number is to say so and change nothing. |
...
/// exactly the ones `pdfce-render` would paint, which means Table 13's
...
fn source_samples(img: &ImportedImage) -> Result<SourceSamples, ImageImportError> {
    ...
    if img.filter == ImportFilter::DctDecode {
        let (components, space) = match coded.color_model {
            ...
            // three-component JPEG pdfce is choosing the space for is the
    ...
    match &img.color_space {
        ...
        ImportColorSpace::Indexed { hival, lookup } => {
            ...
            for &idx in &flat {
                ...
                // producer bug pdfce reads the same way every viewer does.
