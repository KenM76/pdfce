...
//! tag combination it declares is one pdfce can represent faithfully, and
...
//!
//! **pdfce inverts the samples.** This is not the "Adobe CMYK inversion" R29
...
//! transparent, the darker — which reads as "pdfce lost the transparency"
...
//! have no PDF-writable counterpart at all — pdfce writes no LZW and no
//! run-length encoder (R28) — and even Deflate, which pdfce **can** write,
...
//! "pdfce re-compressed my file" is never something to discover by diffing
//! bytes.
//!
...
//! | Other compressions | `TIFF/thunderscan` … `TIFF/unknown-compression` | Need a decoder pdfce does not have. Rule 13 makes adding one a licence-classified decision, not something a feature Pass does in passing. |
//! | Photometric 4, 5, 6, 8–10, 32803, 34892 | `TIFF/photometric-cmyk` … | CMYK is the near miss — `/DeviceCMYK` exists and the ink polarity already agrees — but it carries its own `InkSet`/`NumberOfInks`/`DotRange` surface, and `YCbCr`/`CIELab` need colour conversions pdfce would then own. Left out of the baseline deliberately. |
...
//! output of a sheet-fed scanner. pdfce places the **first** page and reports
...
//! the heuristic in [`palette_from_colormap`] instead. pdfce applies it too,
...
//! only because this module cannot reach `pdfce-core`'s settings surface.
...
/// The largest number of IFDs pdfce will walk while counting pages.
///
/// A pdfce ceiling, not a TIFF one (`ARCHITECTURE.md` §10.1): the chain is a
...
/// # Errors
///
/// See [`ImageImportError`]. Every tag combination pdfce declines is
...
/// what pdfce must do to the samples because of it.
...
enum Colour {
    ...
    Palette {
        ...
        /// definition, and pdfce used them as stored — see the module docs'
        /// recorded ambiguity.
        assumed_8bit: bool,
    },
}
...
/// Everything one page of a TIFF declares, after every combination pdfce
/// cannot represent has already been refused.
///
/// Built by [`Raster::read`], which is where all validation lives — so by the
/// time [`assemble`] runs, "can pdfce do this?" is already answered and the
...
/// Map a `Compression` value pdfce declines to its stable feature key.
///
...
/// Map a `PhotometricInterpretation` pdfce declines to its stable key.
...
/// Map a `SampleFormat` pdfce declines to its stable key (TIFF 6.0 TechNote 3).
///
...
fn decode_strips(
    ...
    for s in 0..strips {
        ...
        // rather than refused for a tag that carries no information pdfce
...
mod tests {
    ...
    fn lzw_encode(data: &[u8]) -> Vec<u8> {
        ...
        // `pdfce-core`'s manifest documents for the LZW filter's fixtures.
    ...
    fn a_transform_of_any_kind_cancels_the_passthrough() {
        ...
        // LZW: pdfce writes no LZW encoder (R28), so nothing to reuse.
    ...
    #[test]
    fn structural_variants_pdfce_cannot_represent_are_refused_by_name() {
    ...
    /// Every truncation must be a diagnosis, never a panic. `pdfce-core`
