...
//! exactly one shape that pdfce chose.
//!
...
//! > to describe them. pdfce decodes and re-compresses only when the PDF
...
//! different in the two formats and cannot be reused. pdfce has no
...
//! array.** This is R29 applied to the write direction: *"pdfce never
...
//! [`ImportNotes::cmyk_polarity_unverifiable`]. pdfce is passing through
...
//! `/Decode`. `pdfce-spec-librarian` should be dispatched for §11.6.5.3
...
///
/// Every variant names the specific thing pdfce would not do. There is no
/// generic "could not read image" — see the module docs on refusing by name.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
#[non_exhaustive]
pub enum ImageImportError {
    /// The file is a raster image in a format pdfce does not place.
    ///
    /// `format` is the format pdfce *recognised* (`"TIFF"`, `"GIF"`, …), so
    /// the operator is told what they actually handed over rather than that
    /// something unspecified went wrong.
    #[error("pdfce does not place {format} images — it places {SUPPORTED_FORMATS}")]
    UnsupportedFormat {
        /// The recognised container format's short name.
        format: &'static str,
    },

    /// The bytes are not a raster image pdfce recognises at all.
    #[error(
    ...
    #[error("this image uses {feature}, which pdfce cannot place")]
    Unsupported {
    ...
    /// The image exceeds pdfce's decode ceilings ([`MAX_IMAGE_PIXELS`],
    /// [`MAX_IMAGE_DIMENSION`], [`MAX_IMAGE_SAMPLE_BYTES`]).
    ///
    /// The same ceilings the *decode* path enforces, deliberately: an image
    /// pdfce would refuse to render is an image pdfce should refuse to
    /// place, or placing it would produce a document pdfce cannot open.
    #[error("this image exceeds pdfce's size ceilings ({MAX_IMAGE_PIXELS} pixels maximum)")]
    TooLarge,
...
/// inventing colorimetry pdfce does not have. `CalRGB`/`Lab`/`ICCBased`/
/// `Separation`/`DeviceN` are all reachable *from* a PDF but not *to* one
/// from a PNG or a JPEG — choosing one would be pdfce asserting a
...
pub enum ImportColorSpace {
    ...
    /// pdfce reads (PNG `PLTE`, BMP's `bmiColors`) are RGB triples. `hival`
...
/// uncompressed image stream is never the right answer for a file pdfce is
/// adding to.
...
/// Why pdfce decoded and re-compressed instead of passing bytes through.
///
...
///
/// pdfce never *applies* this silently — see
...
pub enum DpiSource {
    ...
    /// point) is pdfce's stated assumption, not the file's claim.
    #[default]
    Assumed,
...
/// pdfce bug rather than as a property of the file.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
#[non_exhaustive]
...
pub struct ImportNotes {
    /// pdfce decoded and re-compressed rather than passing the source bytes
    ...
    /// pdfce did **not** carry over. The image is placed in a device colour
    ...
    /// Embedding a profile pdfce cannot validate would be exactly the
...
/// it.
///
/// pdfce does **not** rewrite the document's `%PDF-x.y` header to
...
impl ImportedImage {
    ...
    /// the resulting size is something pdfce *inferred* from metadata the
    ...
    /// so "the file said 300 dpi" and "pdfce assumed 72" are distinguishable.
    ///
...
pub fn sniff(data: &[u8]) -> Result<ImageFormat, ImageImportError> {
    ...
    // codestream's SOC+SIZ. pdfce *reads* JPXDecode images already, but
...
///   — the file is not something pdfce places, named either way.
/// - [`ImageImportError::Unsupported`] — a supported container using a
///   sub-feature pdfce declines (interlaced PNG, arithmetic JPEG, RLE BMP),
...
///
/// The samples must be *exactly* the ones `pdfce-render` would paint, which
...
/// components"* — which, for a three-component JPEG pdfce is choosing the
...
/// pdfce would refuse to render produces a document pdfce cannot display,
...
/// trade, and it is the *only* branch where pdfce chooses a compression
