...
//!
//! `pdfce-render` does **not yet composite `/SMask`** — it counts the
//! entry as a deferred feature and paints the base image opaque. So a
//! correctly-authored transparent PNG looks opaque in pdfce's own preview
...
//! pdfce cannot carry over, so it is disclosed).
//!
...
/// Adam7 file, which pdfce declines rather than mis-decoding.
pub fn import(data: &[u8]) -> Result<ImportedImage, ImageImportError> {
    ...
    if png.interlaced {
        ...
        // formats, and reusing it would produce stripes. pdfce has no
...
/// *ancillary* chunks pdfce does not even read, and refusing to place an
...
/// pdfce cannot express would be wrong, but so would refusing to place a
...
/// the same picture, so they are left alone. pdfce decodes to *read* the
...
/// which is an encoder heuristic pdfce would then own forever, and the
...
fn split_alpha(png: &Png, mut notes: ImportNotes) -> Result<ImportedImage, ImageImportError> {
    ...
    Ok(ImportedImage {
        ...
        soft_mask: Some(SoftMask {
            ...
            // 16-bit alpha, and quantizing it to 8 would be pdfce discarding
