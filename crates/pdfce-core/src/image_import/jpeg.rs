...
//! only: pdfce writes none of these codecs. No image encoder enters any
//! pdfce crate without a new decision record"* — means a JPEG *encoder*
//! cannot be added by a feature Pass at all. Passthrough introduces none:
//! the bytes were encoded by whatever produced the file, and pdfce only
...
//!
//! **R29**: *"pdfce never applies an 'Adobe CMYK inversion.' … `/Decode` is
...
//!
//! pdfce therefore **embeds it as-is and says so**. The alternative — a
...
//! rotated picture — which reads as a pdfce bug, not as a property of the
//! file. Rotating the pixels means a decode and re-encode, which is the
//! generation loss and the forbidden encoder again.
//!
//! So pdfce reads the tag and folds the rotation into the `cm` matrix
...
//! | APP14 transform ≥ 3 | `JPEG/adobe-transform-N` | Outside Table 13's 0–2; pdfce's own decoder rejects it, so placing one would make a document pdfce cannot display |
//!
...
pub fn import(data: &[u8]) -> Result<ImportedImage, ImageImportError> {
    ...
    // Marker outranks `/DecodeParms` (which pdfce does not write) outranks
    ...
    let mut notes = ImportNotes {
        ...
        // — which pdfce never writes (R29), so the third condition is
    ...
    loop {
        ...
        match marker {
            ...
            0xEE => {
                ...
                {
                    if transform > 2 {
                        // Outside Table 13's 0/1/2. pdfce's own decoder
                        // refuses it (R27), so placing one would produce a
                        // document pdfce cannot render — the one outcome
...
/// Read the EXIF IFD0 tags pdfce uses: `Orientation` (0x0112),
