...
/// there, or is not something pdfce models, the only alternatives to
...
pub enum SpecReadError {
    /// The `/Subtype` is not one [`build_appearance`] can author, so pdfce
    ...
    /// path.
    #[error(
        "/Subtype /{subtype} is not a markup type pdfce can author, so its appearance cannot be \
    ...
    /// from geometry pdfce could not parse would mean inventing the
    /// missing half.
    #[error(
        "the annotation's /{key} geometry is missing or malformed, so there is nothing to \
         regenerate an appearance from and pdfce will not guess it"
    )]
    BadGeometry {
...
/// * `/LE` — Table 176 line endings. A value outside the three pdfce
...
/// # Errors
///
/// [`SpecReadError`] — an unmodelled subtype, or geometry pdfce will not
/// guess at.
...
/// Read `/LE` (Table 176), degrading anything outside the three pdfce
