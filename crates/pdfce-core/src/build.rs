...
//! `crates/pdfce-core/build.rs` and read with `env!`, so reading it costs
...
//!
//! # Why it lives in `pdfce-core` and not in each binary
//!
//! Because there must be exactly one answer. `pdfce-cli` and `pdfce-gui`
//! both depend on `pdfce-core`, so both report the same stamp by
...
impl BuildInfo {
    ...
    pub const fn current() -> Self {
        Self {
            ...
            built_at: env!("PDFCE_BUILD_TIMESTAMP"),
            revision: env!("PDFCE_BUILD_REVISION"),
            committed_at: env!("PDFCE_BUILD_COMMIT_TIMESTAMP"),
            iccce: env!("PDFCE_ICCCE_PROVENANCE"),
        }
    }
...
mod tests {
    ...
    fn the_build_timestamp_is_a_plausible_rfc3339_instant() {
        ...
        // pdfce did not exist before its founding conversation.
