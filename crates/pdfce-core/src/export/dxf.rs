...
///
/// pdfce is that upstream producer here, so it declines to repeat the
/// mistake. Text on its own layer means the import is a checkbox: turn
/// `PDFCE_TEXT` off and what remains is the geometry to trace. No
...
const TEXT_LAYER: &str = "PDFCE_TEXT";
...
/// hazard.
///
/// `MTEXT` was considered and rejected: it carries formatting pdfce cannot
...
pub struct DxfOutcome {
    ...
    /// different things of the operator. `skipped_text` means *pdfce chose
    /// not to write this* — they set [`DxfText::Omit`], and nothing is
    /// wrong. This means *pdfce could not read it*, which is a fact about
