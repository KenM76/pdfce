...
//! # Purpose
//!
//! `forms.rs` gives pdfce a **read projection** of an `/AcroForm`: a flat
...
//! which one anything meant. pdfce's own reader copes with that only
...
//! deliberately would mean pdfce authoring the very thing its reader treats
...
//! exposes hierarchy authoring; pdfce does, so pdfce needs it.
//!
...
pub enum FormAuthorError {
    ...
    /// pdfce cannot author a tree its own reader would refuse to descend.
...
/// appearance string and as a field-level default — and pdfce cannot tell
...
mod tests {
    ...
    /// one meaning and pdfce takes it at its word.
    #[test]
    ...
    /// The depth bound matches the reader's, so pdfce cannot author a tree
