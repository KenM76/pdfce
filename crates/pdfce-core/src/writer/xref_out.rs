...
//! E1–E14).
//!
//! ## Never normalize: the form is chosen by the input, not by pdfce
//!
...
//! which is exactly why the rule has to be pdfce's own. Emitting a
...
//! *"most readers repair silently and pdfce's own lenient parser will
...
fn write_classic_entry(
    ...
    let (field1, generation, keyword) = match *entry {
        ...
        XrefEntry::InStream {
          ...
          // inside pdfce-core a new variant must break this match, so
...
impl Widths {
    ...
    ///    NEGATIVE RESULT and pdfce never emits it.
...
mod tests {
    ...
    fn classic_entry_is_exactly_twenty_bytes() {
        ...
        // by round-tripping through pdfce's own lenient parser, which
