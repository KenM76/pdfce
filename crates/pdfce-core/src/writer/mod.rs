...
//!    which is precisely why pdfce has to impose the rule itself.
...
//!    hazard. pdfce's structural answer is that signed objects are, by
...
//! writer must recompute it. So pdfce regenerates `ID[1]` exactly when
//! a save writes at least one changed object, and never otherwise; and
//! `ID[0]` changes only when pdfce creates a document it regards as
//! new, which no save mode in this Pass does. A gratuitously
//! regenerated `/ID` is an observable "pdfce touched this file" signal
//! on a file pdfce did not change — R41 territory.
//!
...
//!   for a metadata edit. pdfce adds nothing, in **both** save modes.
//!   (The RAG does recommend synthesizing on a full rewrite; pdfce
...
//!   Rewriting one would be pdfce inventing document identity; passing
...
/// breaking the "objects pdfce didn't logically touch are omitted"
...
/// pdfce leave **no non-suppressible output fingerprint**. No build
...
/// disqualified `oxidize-pdf` as a foundation for pdfce.
///
...
pub enum ProducerPolicy {
    /// Emit `/Producer (pdfce <version>)` into an existing document
    ...
    /// an inconsistency. R41 forbids pdfce writing *its own* identity
    ...
    /// |---|---|---|---|
    /// | `ProducerPolicy::Set` | yes | **no** | pdfce authored it; creating a whole object to hold a producer id is a fingerprint |
...
/// pdfce's output non-reproducible and would be a fingerprint.
#[must_use]
pub fn producer_string() -> String {
    format!("pdfce {}", env!("CARGO_PKG_VERSION"))
}
...
impl SaveOptions {
    ...
    /// (`pdfce-cli`, `pdfce-gui`, the round-trip harness) cannot use a
