...
//!
//! Under `ARCHITECTURE.md` §5, an object pdfce did not logically modify
...
//! - dictionaries pdfce constructs itself: the trailer (§7.5.5), a
...
/// `pdfce-core`'s panic-free policy plus the writer's
...
/// `pdfce-core` the match below is checked exhaustively. That is the
...
/// parsing a file: the only way to build one is for pdfce's own code
...
/// `#00` produces a file pdfce's own strict lexer refuses to reload,
...
///
/// Computed rather than table-indexed because `pdfce-core` denies
