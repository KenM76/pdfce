...
//! head even though it is byte-identical to stage 1's. pdfce follows
...
//!
//! pdfce does not do that, and the reason is worth stating because it
...
pub struct SaveReport {
    ...
    /// change pdfce made on its own initiative. The honesty obligation
    /// differs accordingly.
...
/// use pdfce_core::writer::{DirtySet, SaveOptions, save_incremental};
///
...
pub fn save_full(
    ...
    for num in numbers {
        ...
        match entry {
            ...
            // base file's free list without pdfce inventing one. A
    ...
    // it avoids re-deriving a free-list chain pdfce has no business
...
/// would rewrite free entries pdfce was not asked to touch (§5). Front
...
/// tidy chain across them would be pdfce inventing structure nobody
...
/// Re-serialize the document information dictionary with pdfce's
...
mod tests {
    ...
    fn incremental_save_never_rewrites_info_r41() {
        ...
        assert_eq!(count_occurrences(&out, b"pdfce "), 0);
    ...
    fn appended_trailer_has_prev_and_exactly_one_of_it() {
        ...
        // pdfce's own parser rejects at parse time.
        let d = doc();
        let (out, _) = save_incremental(
            &d,
            &DirtySet::identity_reemission([ObjId::new(3, 0)]),
    ...
    fn producer_policy_is_suppressible_and_effective() {
        // The R41 acceptance criterion, from the pdfce-core side.
        let d = doc();
        let (preserved, rep) = save_full(&d, &DirtySet::empty(), &SaveOptions::identity()).unwrap();
        assert_eq!(count_occurrences(&preserved, b"pdfce "), 0);
