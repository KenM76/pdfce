...
//! **formula**. Form values come from a PDF that pdfce did not write and
...
//! is a capability pdfce refuses itself (R12) and would here be handing to
//! another program on the operator's behalf.
//!
//! pdfce **neutralises** such a value by prefixing a single apostrophe, the
...
//!   apostrophe it recognises as pdfce's own, so a round trip through a
...
///
/// The same character `pdfce-cli fill-field` uses (`--set Colours=Red|Blue`),
/// so the two surfaces spell a multi-selection identically instead of each
/// choosing its own.
pub const MULTI_SEPARATOR: &str = "|";

/// A CSV export, with what pdfce had to change to make it safe to open.
...
pub struct CsvExport {
    ...
    /// conclude pdfce corrupted something.
...
/// exporting and re-importing pdfce's own output round-trips, and so does a
...
pub fn parse_csv(bytes: &[u8]) -> Result<FormData, CsvError> {
    ...
    // matching fields and look like pdfce could not read it.
    ...
    while let Some(c) = chars.next() {
        ...
        match c {
            ...
            '\r' => {
                ...
                // because a file that reached pdfce through an old tool is
    ...
    pub fn message(&self) -> Option<String> {
        ...
        Some(format!(
            "{} value(s){named}{more} begin with a character a spreadsheet reads as a \
             FORMULA, so pdfce prefixed each with an apostrophe to keep it text. The PDF \
...
mod tests {
    ...
    /// `=WEBSERVICE(…)` fetches. pdfce refuses network access itself (R12);
