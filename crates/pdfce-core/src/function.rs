...
//!
//! Nothing in pdfce could evaluate a function before this module, and that
...
//! `pdfce-core` rather than beside any one consumer. A second, specialised
...
//!
//! So the failure behaviour here is **pdfce policy, chosen deliberately**, and
...
//! | [`MAX_PS_STEPS`] | 1,000,000 | **pdfce policy** — no spec equivalent |
//! | [`MAX_PS_NESTING`] | 32 | **pdfce policy** — no spec equivalent |
//! | [`MAX_SAMPLED_INPUTS`] | 8 | **pdfce policy**, spec-sanctioned (§7.10.2) |
//! | [`MAX_FUNCTION_DEPTH`] | 8 | **pdfce policy** — no spec equivalent |
//!
...
//! [`MAX_PS_STEPS`] is pdfce's answer to that, and it is a policy number that
...
///
/// Both halves matter. 100 is a floor pdfce must meet, and it is simultaneously
/// a ceiling pdfce is entitled to stop at — so overflowing at exactly 100 with
...
/// Maximum operators a single type 4 evaluation may execute — **pdfce policy**.
///
...
/// Maximum `{ }` nesting depth inside a type 4 program — **pdfce policy**.
///
...
/// recurses one frame per `{`, and `pdfce-core`'s panic-free policy treats a
...
/// Maximum input dimensionality of a type 0 sampled function — **pdfce policy,
...
/// Maximum nesting depth of type 3 stitching sub-functions — **pdfce policy**.
///
...
pub enum FunctionError {
    ...
    #[error(
        "sampled function has {got} inputs; pdfce's limit is {limit} (§7.10.2 permits an implementation limit)"
    )]
    TooManyInputs {
        /// The declared *m*.
        got: usize,
    ...
    #[error(
        "function nesting exceeded pdfce's depth limit of {limit} (cycle, or pathologically nested /Functions)"
    )]
    NestingTooDeep {
        /// [`MAX_FUNCTION_DEPTH`].
        limit: usize,
    ...
    /// Execution passed [`MAX_PS_STEPS`] (pdfce policy — see that constant).
    #[error("type 4 program exceeded pdfce's {limit}-step execution cap")]
    StepLimit {
        /// [`MAX_PS_STEPS`].
        limit: usize,
    },
...
impl FunctionType {
    ...
    ///
    /// ```
    /// use pdfce_core::function::FunctionType;
...
impl PdfFunction {
    ...
    /// spec does *not* let pdfce ignore.
    ///
    /// This is disclosure plumbing for project rule 4 (fuzzy, never sneaky).
    /// pdfce evaluates every sampled function multilinearly. §7.10.2 permits
    ...
    ///
    /// The reason pdfce does not simply implement it: §7.10.2 names *"cubic
...
/// with equality allowed — a `/Domain [0.5 0.5]` is legal). pdfce returns
...
/// exactly `ymin` for that collapsed interval. Generalising it is pdfce policy,
...
/// panics when `lo > hi`, and `pdfce-core`'s panic-free policy does not accept
...
/// ## Interpolation
///
/// pdfce interpolates multilinearly (2^m corners, weights the product of each
...
struct Sampled {
    ...
    samples: Vec<u8>,
    /// `/Order 3` was requested, cubic was possible, and pdfce did it linearly
...
/// ([`FunctionError::UnknownOperator`]) rather than a pdfce gap to be silently
...
/// in braces `{ }`."* pdfce enforces that literally — a stream whose first
...
///
/// pdfce fixes it at **32**, because Annex C Table C.1's `integer` row is the
...
/// make pdfce compute a value where a conforming reader raises `typecheck`.
///
/// # Errors
///
...
mod tests {
    ...
    /// pdfce's. Catches the limit silently not being enforced, which would let a
    ...
    /// being dropped, which would let the operator assume a fidelity pdfce does
    /// not provide.
    #[test]
    ...
    /// `pdfce-core`'s panic-free policy treats as seriously as an `unwrap`.
    #[test]
    ...
    /// pdfce's step cap (policy, not spec — see [`MAX_PS_STEPS`]). Catches the
    ...
    /// intuitive-but-wrong "floor returns an integer", which would make pdfce
    ...
    /// pdfce compute where a conforming reader raises.
    #[test]
    ...
    /// `n = 0` leaves `j mod n` undefined (ambiguity `F-A4`); pdfce treats it as
