...
//!   syntax **is** COS syntax, the reader reuses pdfce's own
...
//! dispatch `pdfce-cli fill-field` already does — so importing the same FDF
...
/// exact for the Latin/UTF-16 values pdfce authors.
...
impl FormData {
    ...
    /// trailer `/Root`), parses it with pdfce's own [`Parser`], and flattens
