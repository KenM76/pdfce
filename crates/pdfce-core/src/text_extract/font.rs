...
//! `pdfce-render`'s `text::load`, and it is deliberately a **separate
...
//! at all. Merging them would force `pdfce-core` to depend on a font
...
//!
//! | Rung | Precondition | pdfce status |
//! |---|---|---|
//! | **1** `/ToUnicode` | the entry is present in the font dictionary — **any** `Subtype`, no other test | **complete** ([`super::cmap`]) |
//! | **2** simple + standard names | `/Encoding` is `MacRomanEncoding`/`MacExpertEncoding`/`WinAnsiEncoding`, **or** `/Differences` contains only Adobe-standard-Latin ∪ `Symbol` names | **complete** (Annex D.2 + AGL, both already in `pdfce-core`) |
//! | **3** composite + known collection | a Table 118 predefined CMap **except `Identity-H`/`Identity-V`**, **or** a descendant CIDFont in `Adobe-GB1`/`-CNS1`/`-Japan1`/`-Korea1` | **structural only** — the `registry-ordering-UCS2` CMaps are Adobe resource files pdfce does not bundle. Diagnosed by name, never silently skipped. |
...
//! glyphs mean." When pdfce meets such a font it reports
...
//!    fallthrough. pdfce falls through to rung 2 per code. Every real
...
//!    letter of the clause. pdfce still resolves the codes whose names
...
//!
//! ## What `pdfce-core` cannot see, and says so
//!
...
//! parser. R21 keeps that parser in `pdfce-render`, so extraction cannot
//! read it. pdfce substitutes `StandardEncoding` as the base table,
...
pub enum LadderRung {
    ...
    CidCollection,
    /// pdfce's documented glyph-name extension: the font failed method
    ...
    /// determine what the character code represents". pdfce emits
...
impl LadderRung {
    ...
    /// `false`: the first is pdfce's own recovery, the second is the
...
/// R27: a rung pdfce cannot climb is reported by name, never silently
/// skipped.
...
pub enum Rung3Gap {
    ...
    /// CIDs to Unicode is an Adobe resource file pdfce does not bundle.
    Ucs2NotBundled {
    ...
    /// A Table 118 predefined CMap other than the Identity pair. pdfce
...
pub enum FontNote {
    ...
    /// `pdfce-core` cannot read (R21 — see the module docs).
    ...
    /// consistency a `shall` and states no recovery (N1); pdfce
...
impl ExtractFont {
    ...
    fn resolve_composite(
        ...
        // file pdfce does not bundle: 2 bytes is the right guess for
    ...
    /// codespace is a single 2-byte range). pdfce consumes it as a high
    ...
    pub(crate) fn to_unicode(&self, code: u32, sentinel: UnmappableCode) -> (String, LadderRung) {
            ...
            && let Some(text) = fontdata::glyph_name_to_unicode_string(name)
        {
            let rung = if self.rung2_precondition {
                ...
                // precondition, so this is pdfce's extension, not the
                // standard's rung 2.
...
fn simple_widths(
    ...
    // `pdfce-core` cannot read (R21). A table of zeros would collapse
    // every glyph on a line to one x-coordinate and destroy the derived
    // word/line segmentation entirely, so pdfce estimates from the
...
mod tests {
    ...
    fn rung_sourcing_classification() {
        ...
        // pdfce's own recovery and the standard's own defeat are NOT
