...
//!    to define the carry. pdfce refuses past the bound and counts it
...
//!   recovery — so pdfce fixes the precedence itself and records the
...
//!   clause states no recovery — pdfce skips them and counts
...
//! (§9.10.3 N2). pdfce accepts a name destination and resolves it
...
//!
//! Every ceiling here is an explicit pdfce constant with a stated
...
/// longer destination is non-conforming; pdfce truncates nothing and
...
/// disclose it instead of pdfce quietly absorbing it.
///
...
pub struct CMapStats {
    ...
    /// known-incomplete.
    pub truncated: bool,
    /// `usecmap` references, which pdfce recognizes but does not follow
    /// (a `ToUnicode` CMap based on another one is legal per §9.10.3 but
    /// requires resolving a name to a bundled resource pdfce does not
    /// carry).
...
impl ToUnicodeCMap {
    ...
    /// precedence, so pdfce picks the rule a sequential reader falls
    ...
    /// every real reader implements is therefore pdfce *policy*,
...
/// Skip a `begin…`/`end…` block whose contents pdfce does not consume.
...
CMapName currentdict /CMap defineresource pop
    ...
    fn overlapping_ranges_are_last_wins() {
        ...
        // precedence is stated. pdfce documents last-wins; this pins it.
        let cmap = ToUnicodeCMap::parse(
