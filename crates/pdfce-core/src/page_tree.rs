...
/// Maximum page-tree nesting depth (pdfce policy, ARCHITECTURE.md
/// §10): legitimate trees are shallow (balanced fan-out ~25–50); a
/// deeper chain is damage or hostility.
pub const MAX_TREE_DEPTH: usize = 64;

/// Maximum number of pages walked (pdfce policy, ARCHITECTURE.md
...
pub enum PageTreeError {
    ...
    Cycle(ObjId),
    /// Nesting exceeded [`MAX_TREE_DEPTH`] (pdfce guard).
    #[error("page tree exceeds MAX_TREE_DEPTH ({MAX_TREE_DEPTH})")]
    TooDeep,
    /// More than [`MAX_PAGES`] leaves (pdfce guard).
...
impl InheritedRaw {
    ...
    /// restating it would modify an object pdfce was not asked to
    /// modify (§5).
    #[must_use]
