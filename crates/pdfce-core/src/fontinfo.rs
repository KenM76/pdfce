...
//! anywhere on screen. pdfce says why. That is a deliberate divergence, and
...
//!    pdfce has already parsed, and it is the number an operator optimising
...
//! and nothing speaks to the read-only Fonts tab — so pdfce states its own
...
pub enum FontSubtype {
    ...
    CidFontType0,
    /// A `/Subtype` name pdfce does not model, carried verbatim.
    Other(String),
...
impl FontSubtype {
    ...
    ///
    /// ```
    /// use pdfce_core::fontinfo::FontSubtype;
    ///
...
pub enum Encoding {
    ...
    /// An **embedded CMap stream** (§9.7.5.3). pdfce does not parse it
...
impl Encoding {
    ...
    ///
    /// ```
    /// use pdfce_core::fontinfo::Encoding;
    ///
...
pub enum EmbeddingPermission {
    ...
    /// pdfce reports the ambiguity rather than resolving it.
    Ambiguous,
...
pub struct FsTypeBits {
    ...
    /// value that was actually read rather than only pdfce's reading of it.
    pub raw: u16,
...
/// specification states no default for the absent case (N1); pdfce
...
impl FsType {
    ...
    /// expression of "pdfce does not guess a permission bit".
    #[must_use]
...
///
/// `pdfce-render` already reads `fsType`, via `skrifa`, in
/// `font::subset::check_embedding_permission`. That crate boundary is
/// load-bearing (project rule 2 / `ARCHITECTURE.md` §3): `pdfce-core` must
/// not gain a font-parsing dependency, because `pdfce-core` is what the
...
///
/// ```
/// use pdfce_core::fontinfo::{read_fs_type, EmbeddingPermission, FsTypeError};
///
/// // Not a font at all.
...
/// exact value, so a font pdfce shows as blocked and a font pdfce declines
...
pub enum Removability {
    ...
    /// arbitrary graphics.
    BlockedType3,
    /// pdfce cannot tell. Reported as its own state rather than folded into
...
pub enum RemovabilityUnknown {
    ...
    /// document.
    PredefinedCMap,
    /// A composite font with an **embedded** CMap stream. pdfce does not
    ...
    NoDescendant,
    /// The `/Subtype` is absent or one pdfce does not model.
    UnknownSubtype,
}
...
impl Removability {
    ...
    ///
    /// ```
    /// use pdfce_core::fontinfo::Removability;
    ///
    ...
    ///
    /// Written as a statement about **the file**, never about pdfce's
    /// abilities: "this font's text is stored as glyph indices" is
    /// actionable; "pdfce cannot remove this" is not.
    #[must_use]
    pub const fn reason(&self) -> &'static str {
        match self {
            ...
            Self::Unknown(RemovabilityUnknown::EmbeddedCMap) => {
                "This font's encoding is an embedded CMap stream, which pdfce does not \
            ...
            Self::Unknown(RemovabilityUnknown::UnknownSubtype) => {
                "This font dictionary declares no /Subtype, or one pdfce does not model, so \
...
/// not merely available. The one surface pdfce does not walk
...
impl SurfaceCoverage {
    /// What this build of pdfce actually walks.
    ...
    ///
    /// ```
    /// use pdfce_core::fontinfo::{Surface, SurfaceCoverage};
...
impl FontRecord {
    ...
    ///
    /// ```
    /// use pdfce_core::fontinfo::split_subset_tag;
    ///
...
/// `AB+Condensed` exists as a possibility and mangling it would be pdfce
...
///
/// ```
/// use pdfce_core::fontinfo::is_standard_14;
///
...
/// ```
/// use pdfce_core::document::Document;
/// use pdfce_core::fontinfo::{inventory, Removability};
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// let doc = Document::from_bytes(
...
/// 2. **Unreadable program** next, because a font pdfce could not read is a
///    font pdfce cannot classify — and saying "removable" about one would
...
mod tests {
    ...
    /// name, not a malformed tag, and pdfce must not invent a family name
    /// by trimming one.
    #[test]
    ...
    fn fs_type_decoding_edge_cases() {
        ...
        // Two usage bits at once: non-conforming from v3, and pdfce reports
    ...
    /// The hand-written reader must agree with `pdfce-render`'s
    /// `skrifa`-based one. They exist separately only because `pdfce-core`
    ...
    ///
    /// The cross-check itself lives in `pdfce-render`'s test suite, where
    ...
    /// would be pdfce asserting something it has not established.
    #[test]
    ...
    /// excludes it — so covering it is a divergence pdfce states rather than
    /// assumes.
    #[test]
    ...
    /// Coverage is reported, and the surface pdfce does NOT walk is named.
    #[test]
    fn coverage_names_what_was_not_searched() {
        ...
        assert_eq!(
            ...
            "the one surface pdfce does not walk must be named, not omitted"
        );
    ...
    /// no fonts" and "pdfce could not look" would print identically, which
