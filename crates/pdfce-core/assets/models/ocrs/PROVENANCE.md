...
first hypothesis was that pdfce was running a checkpoint as its recogniser.
...
it.** The survey's own next sentence — *"pdfce must pin exactly which artifact
...
pdfce's position, stated rather than assumed: both files are the same author's
...
**Changes made by pdfce: still NONE.** Both files are byte-identical to their
