...
//! had to *describe* it, because `find` was required. `pdfceGUI` reported
...
//!    operator pdfce chose for them.
//! 3. **It is disclosed.** The extent was chosen by pdfce, not typed by the
...
//!    show operators (13 % of runs over pdfce's corpus; see
...
fn edit_text_gets_the_same_affordance() {
    ...
    let outcome = pdfce_core::text_edit::edit_text(&d, &req, &EditOptions::default())
    ...
    let pages = pdfce_core::page_tree::pages(&after).expect("pages");
...
fn an_empty_find_with_no_pin_is_still_refused() {
    ...
    // restyles whichever operator pdfce happened to reach first.
    let d = doc("format_family.pdf");
    ...
    let err = pdfce_core::text_edit::edit_text(
        &d,
...
// ---------------------------------------------------------------------------
// 3. Disclosure (rule 4 — the extent was pdfce's choice, not the operator's)
// ---------------------------------------------------------------------------
