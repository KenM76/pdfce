...
use pdfce_core::forms::{self, FieldFlags};
...
fn shortening_max_len_below_the_stored_value_discloses_and_does_not_truncate() {
    ...
    assert_eq!(
        ...
        "pdfce must NOT truncate: the data is the operator's, and silently \
...
fn border_and_visibility_are_widget_scope() {
    ...
    assert_eq!(
        bs.get(b"S")
            .and_then(pdfce_core::object::Object::as_name)
            .unwrap()
            .as_bytes(),
        b"D"
    );
    assert_eq!(
        dict.get(b"F").and_then(pdfce_core::object::Object::as_int),
        Some(36),
