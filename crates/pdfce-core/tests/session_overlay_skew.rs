...
use pdfce_core::image_import::{self, ImportedImage};
use pdfce_core::page_tree::Rect;
use pdfce_core::vector::{Matrix, TextPreview, TransformOptions, VectorObject};
...
fn a_text_edit_does_not_erase_an_image_added_this_session() {
    ...
    let req = pdfce_core::text_edit::EditRequest::find_replace(0, "Page One", "Sheet One");
    s.edit_text(&req, &pdfce_core::text_edit::EditOptions::default())
...
fn reflow_refuses_once_the_page_set_has_changed() {
    ...
    s.delete_pages(&[0]).unwrap();
    let err = s
        .reflow_block(0, 0, &pdfce_core::text_edit::ReflowRequest::default())
