...
//!
//! Drives `pdfce_core::text_edit::add_text` (the free engine) and
//! `pdfce_core::edit::EditSession::add_text` (the undo-able session command)
...
use pdfce_core::edit::{CommandKind, EditSession};
...
fn colour_and_font_choice_are_honoured() {
    ...
    let req = AddTextRequest::new(0, (100.0, 650.0), "Red Times")
        .with_font(pdfce_core::fontdata::Std14::TimesRoman)
...
fn preview_wrap_lines_match_committed_boxed_add_for_identical_inputs() {
    ...
    for (i, &(text, llx, lly, w, h, font, size, align, leading)) in cases.iter().enumerate() {
        ...
        let non_blank: Vec<&pdfce_core::text_edit::WrapPreviewLine> = preview
            .lines
            .iter()
