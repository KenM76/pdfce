...
//! at all. An in-crate test sees none of that: inside `pdfce-core` the
//! attribute is inert, so an in-crate test would happily write a `match` a
//! real consumer cannot compile. These tests are written the way `pdfceGUI`
...
//!    embedding files by `pdfceGUI`'s own survey. `set_font`'s name match
...
/// `if let`. Deliberately not a helper inside `pdfce-core`, where the
