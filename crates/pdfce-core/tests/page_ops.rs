...
//!
//! End-to-end, through the same API `pdfce-gui` and `pdfce-cli` use:
//! [`EditSession`] for the in-place edits, [`pdfce_core::pageops`] for the
...
fn deleting_every_page_of_a_branch_prunes_the_now_empty_node() {
    ...
    assert_eq!(pdfce_core::page_tree::pages(&reloaded).unwrap().len(), 1);
}
...
fn reorder_changes_the_order_without_touching_page_content() {
    ...
    let reloaded = Document::from_bytes(save_incremental(&s)).unwrap();
    let ids: Vec<ObjId> = pdfce_core::page_tree::pages(&reloaded)
        .unwrap()
        .iter()
        .map(|p| p.id)
        .collect();
...
fn reordering_across_ancestors_materializes_what_the_page_used_to_inherit() {
    ...
    let reloaded = Document::from_bytes(save_incremental(&s)).unwrap();
    let after: Vec<u16> = pdfce_core::page_tree::pages(&reloaded)
        .unwrap()
        .iter()
        .map(|p| p.rotate)
        .collect();
...
fn delete_reports_the_bookmarks_and_links_it_orphans() {
    ...
    assert!(
        ...
        "Acrobat leaves labels stale AND silent; pdfce leaves them stale and says so"
    );
...
fn a_metadata_only_edit_on_a_signed_document_reports_byte_range_preserved() {
    ...
    let source = signed_doc(false, "2");
    let mut s = session(&source);
    s.set_info_field(pdfce_core::edit::InfoField::Title, Some("New"))
        .unwrap();
...
fn extract_from_an_edited_session_sees_the_unsaved_edits() {
    ...
    let pages = pdfce_core::page_tree::pages(&out).unwrap();
...
fn merge_then_split_round_trips_the_page_count() {
    ...
    for (part, bytes, _) in parts {
        let out = Document::from_bytes(bytes).unwrap();
        assert_eq!(
            pdfce_core::page_tree::pages(&out).unwrap().len(),
            part.page_count()
        );
    }
}
...
///
/// ```text
/// PDFCE_CORPUS="fixtures/external/veraPDF-corpus" \
///   cargo test -p pdfce-core --test page_ops -- --ignored --nocapture
/// ```
///
...
fn measure_guard_headroom_against_a_corpus() {
    let Ok(root) = std::env::var("PDFCE_CORPUS") else {
        eprintln!("PDFCE_CORPUS not set — nothing to measure");
        return;
    };
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            let resolver = pdfce_core::pageops::DestinationResolver::new(&doc);
            max_named_dests = max_named_dests.max(resolver.named_count());

            if let Ok(slots) = pdfce_core::page_tree::page_slots(&doc) {
                ...
                .and_then(Object::as_dict)
            {
                ...
                let mut seen = 0usize;
                pdfce_core::pageops::references::walk_outline(
                    &doc,
