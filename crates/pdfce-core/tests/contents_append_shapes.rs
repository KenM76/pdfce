...
//! document pdfce itself refuses to open.
//!
...
use pdfce_core::edit::{EditSession, NewImage};
...
fn tiny_image() -> pdfce_core::image_import::ImportedImage {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/synthetic/images/rgb8.png");
    let bytes = std::fs::read(&path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
    pdfce_core::image_import::import(&bytes).expect("import")
}
...
fn add_text_survives_an_indirect_array_contents() {
    use pdfce_core::text_edit::AddTextRequest;
...
// ---------------------------------------------------------------------------
// Reading back a file a SHIPPED pdfce already damaged
// ---------------------------------------------------------------------------
...
/// Refusing to open them would cost him work pdfce itself destroyed.
///
...
fn a_page_damaged_by_an_older_build_still_opens_and_is_disclosed() {
    ...
    // ★ And it is DISCLOSED, not silently healed. Rule 4: pdfce repaired
    // something, so pdfce says so.
    assert_eq!(
