...
fn copying_pages_leaves_the_source_alone_and_yields_a_real_document() {
    ...
    assert_eq!(
        pdfce_core::page_tree::pages_in(&reopened)
            .expect("page tree")
            .len(),
        1,
...
fn a_page_clip_crosses_documents_through_its_bytes() {
    ...
    let carried = pdfce_core::pageops::PageClip::from_bytes(clip.to_bytes().to_vec())
...
fn a_page_copy_reports_the_fields_it_could_not_carry() {
    ...
    assert!(
        pdfce_core::forms::parse_acroform(&destination.graph()).is_none_or(|f| f.fields.is_empty()),
...
fn a_page_clip_that_is_not_a_pdf_is_refused() {
    let mut s = session("hello.pdf");
    assert!(
        pdfce_core::pageops::PageClip::from_bytes(b"not a pdf at all".to_vec()).is_err(),
