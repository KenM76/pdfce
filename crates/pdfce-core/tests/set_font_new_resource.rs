...
use pdfce_core::graph::ObjectGraph as _;
use pdfce_core::object::{Dict, Object};
use pdfce_core::text_edit::{FontSelector, FormatOptions, FormatRequest};
...
fn page_text(doc: &Document) -> String {
    let pages = pdfce_core::page_tree::pages(doc).expect("page tree");
    let pt = pdfce_core::text_extract::extract_page(
        doc,
        &pages[0],
        0,
...
fn the_one_shot_path_writes_the_resource_it_says_it_added() {
    ...
    let outcome =
        pdfce_core::text_edit::set_format(&doc, &request("Helvetica"), &FormatOptions::default())
...
fn the_disclosure_and_the_file_agree_about_the_new_key() {
    let doc = Document::load(&fixture()).expect("load fixture");
    let outcome =
        pdfce_core::text_edit::set_format(&doc, &request("Courier"), &FormatOptions::default())
...
fn a_non_standard_14_face_is_still_refused_by_name() {
    ...
    let err = pdfce_core::text_edit::set_format(&doc, &request("Arial"), &FormatOptions::default())
...
/// because pdfce built it. A refusal naming `TargetFontMissing` here would mean
/// the synthesis never happened; a SUCCESS here would mean pdfce wrote
...
fn a_standard_14_face_that_cannot_show_the_run_is_refused_on_coverage() {
    let doc = Document::load(&fixture()).expect("load");
    let err =
        pdfce_core::text_edit::set_format(&doc, &request("Symbol"), &FormatOptions::default())
...
fn a_face_already_on_the_page_adds_nothing() {
    ...
    let outcome =
        pdfce_core::text_edit::set_format(&doc, &request("Times-Roman"), &FormatOptions::default())
...
/// bound into. `pdfceF1` is picked as the first unused name, and the `pdfce`
...
/// with no built-in standard-14 metrics still lays it out correctly. pdfce
...
fn the_authored_font_dictionary_carries_widths_and_an_encoding() {
    ...
    let saved = Document::from_bytes(outcome.bytes).expect("re-parse");
    let view = saved.view();
    let pages = pdfce_core::page_tree::pages(&saved).expect("page tree");
    let fonts = pages[0]
        .resources
        .get(b"Font")
        .map(|o| view.resolve(o))
