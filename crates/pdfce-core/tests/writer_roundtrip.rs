...
//!
//! Whole-file coverage of `pdfce-core`'s two save modes (ISO 32000-1
...
//! hybrid-reference file". So the fixtures below are pdfce's own,
...
use pdfce_core::object::{ObjId, Object, Provenance, equivalent_across_buffers};
use pdfce_core::writer::{
    DirtySet, ProducerPolicy, SaveOptions, WriteError, save_full, save_incremental,
};
use pdfce_core::xref::{SectionShape, XrefEntry};
...
/// See `pdfce_core::object::equivalent_across_buffers`.
...
fn classic_append_stays_classic_even_though_the_spec_would_allow_a_stream() {
    ...
    // pdfce policy, and policy has to be tested.
    let bytes = build_classic_pdf(&[], true);
...
fn id_is_untouched_by_an_identity_save_of_either_kind() {
    ...
    // regenerated /ID is an observable "pdfce touched this" signal.
    let bytes = build_classic_pdf(&[], true);
    let doc = Document::from_bytes(bytes.clone()).unwrap();
...
fn producer_policy_is_effective_and_suppressible() {
    ...
    assert_eq!(count(&preserved, b"pdfce "), 0);
...
fn incremental_save_never_rewrites_info_regardless_of_policy() {
    ...
    for opts in [SaveOptions::default(), SaveOptions::identity()] {
        ...
        assert_eq!(count(&out, b"pdfce "), 0);
    }
}

// ---------------------------------------------------------------------------
// Cross-reference-stream files (§7.5.8) — pdfce's own fixtures
// ---------------------------------------------------------------------------
...
fn a_live_linearized_file_is_detected_and_the_save_reports_the_loss() {
    ...
    assert_eq!(
        pdfce_core::linearization::detect(&lin),
        Linearization::Live {
            declared_length: total
        }
    );
    assert!(
        pdfce_core::linearization::detect(&lin).save_invalidates_fast_web_view(),
        "a live linearized file must warn on save"
    );
    // pdfce never strips the dictionary and never patches /L.
...
fn a_signature_shaped_object_is_copied_verbatim_never_reserialized() {
    ...
    // "identically" — is a byte-offset hazard, so pdfce's structural
