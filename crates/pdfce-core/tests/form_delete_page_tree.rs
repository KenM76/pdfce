...
//! produces a file pdfce itself cannot reopen. That is the exact shape the
...
/// here — pdfce cannot delete the field without deleting the page, and
...
fn deleting_a_field_that_is_a_page_never_produces_an_unreadable_file() {
    ...
    assert!(
        reloaded.is_ok(),
        "delete_field returned {result:?} and produced a file pdfce cannot reopen"
    );
    let doc = reloaded.unwrap();
    assert!(
        pdfce_core::page_tree::pages(&doc).is_ok(),
...
fn deleting_a_group_that_is_a_page_never_produces_an_unreadable_file() {
    ...
    assert!(
        reloaded.is_ok(),
        "delete_field_group returned {result:?} and produced a file pdfce cannot reopen"
    );
}
...
fn deleting_a_field_that_is_the_catalog_never_produces_an_unreadable_file() {
    ...
    assert!(
        pdfce_core::page_tree::pages(&reloaded.unwrap()).is_ok(),
