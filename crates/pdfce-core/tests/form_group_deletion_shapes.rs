...
/// update, and **the two shells read different ones**: `pdfce-cli` prints
/// `nodes_removed`, `pdfceGUI` prints `nodes.len() - 1`. So the same deletion
...
fn the_outcome_reports_one_number_not_two() {
    for (name, group) in [
        ...
        let Ok(out) = s.delete_field_group(group) else {
            // A refusal is a legitimate answer for a shape pdfce declines to
