...
//! that asserted on the dictionary keys pdfce just inserted would pass for a
...
use pdfce_core::outline::{DestView, Destination, Outline, RemoteTarget, read_outline};
...
/// distinction cost a sabotage run to find: pdfce's reader walks `/First`
...
/// pdfce happened to allocate rather than the real page.
///
        ...
        (
            Destination::Page {
                ...
                view: DestView::Unknown {
                    fit: pdfce_core::object::Name::from(b"FitSomething"),
                },
            },
            "unknown-fit",
        ),
...
fn back_walk(doc: &Document, parent: ObjId) -> Vec<String> {
    ...
    for _ in 0..64 {
        ...
        let title = match d.get(b"Title") {
            Some(Object::String(bytes)) => pdfce_core::edit::decode_text_string(bytes).text.clone(),
