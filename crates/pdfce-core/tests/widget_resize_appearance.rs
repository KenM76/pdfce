...
//!
//! `pdfceGUI`'s request named the `/Btn` route
...
fn a_resized_check_box_is_redrawn_at_the_new_size() {
    ...
    let out = s
        ...
        .expect("pdfce drew this artwork, so pdfce can redraw it");
    assert!(out.resized);
    assert!(
        out.appearance_regenerated,
        "a check box pdfce drew must be rebuilt, not carried"
    );
...
fn state_ids(s: &EditSession, name: &str) -> Vec<(Vec<u8>, pdfce_core::object::ObjId)> {
    let g = s.graph();
    let field = field_named(s, name);
    let dict = g
...
/// A push button pdfce drew is its own artwork too, and its plate carries the
...
// -------------------------------------------------------------------------
// The ownership boundary — pdfce redraws only what pdfce drew
// -------------------------------------------------------------------------

/// ★ The load-bearing refusal. A check box whose artwork somebody else drew
/// must NOT come back as pdfce's two-line vector tick as a side effect of a
/// drag.
///
...
/// which pdfce would never draw. That is what makes this a test of the
...
fn a_foreign_button_appearance_is_not_redrawn() {
    ...
    assert!(
        !out.appearance_regenerated,
        "somebody else's artwork must not be replaced with pdfce's rendering of it"
    );
    assert!(
        out.appearance_stale
            .as_deref()
            .is_some_and(|m| m.contains("pdfce did not draw") && m.contains("NON-UNIFORM")),
