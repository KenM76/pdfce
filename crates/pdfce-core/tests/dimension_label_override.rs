...
fn the_clipboard_carries_the_override_through_a_byte_round_trip() {
    ...
    let round_tripped = pdfce_core::vector::ObjectClip::from_bytes(&bytes).unwrap();

    let mut dest = session();
    let outcome = dest
        .paste_objects(0, &round_tripped, pdfce_core::vector::Matrix::IDENTITY)
        .unwrap();
...
fn the_sidecar_version_rises_only_for_a_document_that_uses_an_override() {
    fn stored_version(session: &EditSession) -> i64 {
        ...
        let pdfce = deref(piece.as_dict().unwrap().get(b"pdfce"));
        let private = deref(pdfce.as_dict().unwrap().get(b"Private"));
        pdfce_core::dimension::sidecar_version(&private).unwrap()
    }
    ...
    assert_eq!(
        stored_version(&s),
        pdfce_core::dimension::SIDECAR_VERSION,
...
fn the_override_round_trips_through_the_sidecar_codec() {
    ...
    let again = pdfce_core::dimension::serialize_model(&model);
