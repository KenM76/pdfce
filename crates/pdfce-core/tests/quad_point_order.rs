...
//! divergence but its **invisibility**: pdfce bakes a full `/AP` (R44), so
...
fn quad_points(annot: &pdfce_core::object::Dict) -> Vec<f64> {
...
/// bow-tie rather than the marked rectangle. pdfce applies its own redactions
...
fn redaction_marks_honour_the_same_setting() {
    ...
    let spec = RedactSpec {
        ...
        fill: None,
        overlay_text: None,
        quadding: pdfce_core::vartext::Quadding::default(),
    };
...
fn the_setting_round_trips_through_the_settings_file() {
    for (token, expected) in [
        ...
        let settings = pdfce_core::settings::Settings::parse(
    ...
    // operator asked for something and pdfce did something else.
    let mut notes = Vec::new();
    let settings =
        pdfce_core::settings::Settings::parse("quad_point_order = widdershins\n", &mut notes);
    assert_eq!(
