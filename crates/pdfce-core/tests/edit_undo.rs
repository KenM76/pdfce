...
//!
//! Whole-file coverage of `pdfce-core`'s **first editing capability**,
...
use pdfce_core::edit::{CommandKind, EditSession, InfoField};
use pdfce_core::object::{ObjId, Object, Provenance};
use pdfce_core::writer::{DirtySet, ProducerPolicy, SaveOptions, save_full, save_incremental};
...
fn page_rotation(doc: &Document, index: usize) -> u16 {
    pdfce_core::page_tree::pages(doc).unwrap()[index].rotate
}
...
fn title(doc: &Document) -> Option<String> {
    ...
    match dict.get(b"Title")? {
        Object::String(bytes) => Some(pdfce_core::edit::decode_text_string(bytes).text),
        _ => None,
    }
}
...
fn non_ascii_metadata_survives_a_save_and_a_reload() {
    ...
    let decoded = pdfce_core::edit::decode_text_string(bytes);
...
fn a_metadata_edit_does_not_stamp_a_producer() {
    ...
    // pdfce's name written into their file (decision 001 §6.1
    ...
    let out = save(&s);
    assert_eq!(
        String::from_utf8_lossy(&out).matches("pdfce ").count(),
        0,
...
fn a_full_rewrite_can_still_stamp_a_producer_alongside_an_edit() {
    ...
    assert!(String::from_utf8_lossy(producer).starts_with("pdfce "));
}
