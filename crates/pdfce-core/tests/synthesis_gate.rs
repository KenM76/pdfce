...
//! who already knew to try a resource pdfce did not offer. `pdfceGUI` found it
//! with a driven test of pdfce's own fixture; the engineer reproduced all
...
fn the_remedy_the_refusal_names_actually_succeeds() {
    ...
    .map(|o| o.report)
    .unwrap_or_else(|e| panic!("the remedy pdfce recommended must work, but: {e}"));
