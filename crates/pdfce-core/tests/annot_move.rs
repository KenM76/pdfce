...
//!    appearance pdfce did not draw survives intact.
...
use pdfce_core::annot_author::{Color, LineEnding, MarkupSpec};
...
fn nums(doc: &Document, d: &pdfce_core::object::Dict, key: &[u8]) -> Vec<f64> {
...
fn rect_of(doc: &Document, d: &pdfce_core::object::Dict) -> Vec<f64> {
    nums(doc, d, b"Rect")
}
...
fn both_halves_move_the_rect_and_the_geometry_keys() {
    ...
    // invisible in pdfce's own rendering. If only `/Rect` moved, this
...
fn the_appearance_stream_is_not_touched() {
    ...
    // replacing an appearance pdfce did not author with pdfce's own drawing
