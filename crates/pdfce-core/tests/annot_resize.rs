...
//! pdfce's Inkscape-parity RAG records that pdfce should be *better than* the
...
//!    behaviour `pdfceGUI` specified — stroke width does not travel — and each
...
use pdfce_core::edit::{EditError, EditSession, ResizeOptions, ResizedAppearance};
...
/// therefore **genuinely foreign** to pdfce — the property three tests below
...
/// A `/Polygon` carries `/Vertices`, which pdfce's own rendering never
...
/// `/Vertices`. The failure is invisible in pdfce and visible in Acrobat.
#[test]
...
/// `pdfceGUI` specified "stroke width does not scale" and argued it from CAD
...
/// pdfce picks the geometric mean — the area-preserving choice — and the point
...
/// An appearance pdfce drew is pdfce's to redraw, so both stroke-width states
/// are exactly satisfiable and no option has to be waived.
#[test]
fn a_pdfce_authored_appearance_is_rebuilt() {
    let (mut s, id) = with_markup(&square());
    let out = s
        .resize_annotation(id, ANCHOR, 2.0, 3.0, &ResizeOptions::default())
        .expect("pdfce redraws its own artwork");
