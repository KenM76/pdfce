...
//! `Highlight` are. pdfce has two authoring verbs — `add_markup` for the
...
use pdfce_core::edit::{EditError, EditSession, MarkupOptions};
...
fn constant_alpha(s: &EditSession, id: pdfce_core::object::ObjId) -> Option<f64> {
    let graph = s.graph();
...
/// changes nothing — and makes a pdfce-authored opaque annotation textually
...
/// caller round-tripping `Some(1.0)` through pdfce must get its key back.
/// Collapsing `Some(1.0)` to `None` "because it renders the same" would be
/// pdfce deciding what the caller meant.
#[test]
...
/// §12.5.2 makes `/CA` composite the *annotation* onto the page, and pdfce's
...
fn the_appearance_stream_can_carry_no_alpha_of_its_own() {
    ...
    let Some(Object::Dict(ap)) = annot.get(b"AP") else {
        panic!("R44: pdfce always authors an /AP");
    };
