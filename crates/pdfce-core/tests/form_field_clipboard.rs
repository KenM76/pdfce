...
//! values pdfce did **not** choose — and every one of them is invisible when
...
use pdfce_core::forms::{self, ButtonKind, FieldType};
...
fn a_pasted_field_keeps_the_appearance_string_quadding_and_length_limit() {
    ...
    assert_eq!(
        pasted.quadding,
        pdfce_core::vartext::Quadding::Center,
...
fn a_pasted_widget_keeps_its_border_colour_background_and_style() {
    ...
    assert!(
        mk.get(b"BG").is_some(),
        "the background colour travelled too -- nothing in pdfce AUTHORS a /BG, \
...
/// Carrying the `/DA` without the font it names produces a field pdfce can
...
fn the_font_the_appearance_string_names_travels_with_the_field() {
    ...
    let parsed = pdfce_core::vartext::parse_default_appearance(&da).expect("well-formed /DA");
    ...
    assert_eq!(
        ...
        Some(&Object::Name(pdfce_core::object::Name::from(
            b"Helvetica-Bold"
        ))),
...
fn a_colliding_font_resource_name_is_renamed_and_the_appearance_string_rewritten() {
    ...
    assert_eq!(
        rival,
        Some(Object::Name(pdfce_core::object::Name::from(b"Courier"))),
...
/// artefact. pdfce declines to make that object rather than making it and
