...
//!
//! `delete_annotation` is the **first** pdfce operation that a `/DocMDP`
...
use pdfce_core::annot::{ReplyType, page_annotations};
use pdfce_core::document::Document;
use pdfce_core::edit::{AnnotationDeletionRoute, EditError, EditSession};
...
fn dict_of(doc: &Document, id: ObjId) -> pdfce_core::object::Dict {
...
fn deleting_a_primary_takes_its_popup_and_unlinks_but_never_deletes_its_replies() {
    ...
    let slots = pdfce_core::page_tree::pages(&doc).expect("pages");
...
/// first, pdfce silently deletes annotations a `shall` forbids deleting —
...
/// bit index both pass, and pdfce would refuse deletions the standard
...
// ---------------------------------------------------------------------------
// The certification gate — the first pdfce operation any /P value permits
// ---------------------------------------------------------------------------
...
fn p3_permits_deleting_a_comment_and_still_refuses_deleting_a_field() {
    ...
    assert!(
        ...
        "/P 3 is 'the same as for 2, as well as annotation creation, \
         deletion, and modification'. This is the first pdfce operation any \
