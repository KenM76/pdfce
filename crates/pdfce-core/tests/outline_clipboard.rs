...
//!
//! pdfce **drops** it and counts it. Not clamped to the last page: a bookmark
...
use pdfce_core::outline::{DestView, Destination, OutlineClip, read_outline};
...
fn with_outline(s: &mut EditSession) -> pdfce_core::object::ObjId {
    let parent = s
...
fn titles(s: &EditSession) -> Vec<String> {
    fn walk(items: &[pdfce_core::outline::OutlineItem], out: &mut Vec<String>) {
        for item in items {
...
fn a_destination_past_the_end_is_dropped_and_counted_not_clamped() {
    ...
    for _ in 0..8 {
        source
            .paste_pages(&page, pdfce_core::pageops::InsertPosition::End)
...
fn a_payload_that_is_not_a_bookmark_clip_is_refused_by_its_signature() {
    assert!(matches!(
        ...
        Err(pdfce_core::outline::OutlineClipError::NotAClip)
    ));
}
