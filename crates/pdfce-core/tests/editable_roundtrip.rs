...
//! qpdf round trip rewrites the whole file. The test that pdfce's compile-back
...
use pdfce_core::editable::{self, EditableError};
use pdfce_core::object::{Name, ObjId, Object};
...
/// The export is a valid PDF that pdfce can reopen, with the same object count.
///
...
fn an_export_is_a_valid_pdf_with_the_same_objects() {
    ...
    assert!(
        pdfce_core::page_tree::pages_in(back.view().graph()).is_ok(),
...
fn an_export_expands_object_streams() {
    ...
    let before = pdfce_core::structure::layout(&doc);
    assert!(
    ...
    let after = pdfce_core::structure::layout(&back);
    assert!(
...
fn a_one_object_edit_appends_only_that_object() {
    ...
    let mut dirty_for_rebuild = pdfce_core::writer::DirtySet::empty();
    dirty_for_rebuild.replace(target, Object::Dict(patched));
    let (rebuilt, _) =
        pdfce_core::writer::save_full(&edited, &dirty_for_rebuild, &SaveOptions::identity())
