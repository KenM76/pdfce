...
use pdfce_core::forms;
use pdfce_core::forms_author::{self, FieldPath, FieldShape, FormAuthorError, resolve_field_path};
...
fn dict_of(s: &EditSession, id: ObjId) -> pdfce_core::object::Dict {
    s.graph()
        .resolved(id)
        .as_dict()
        .cloned()
...
/// half-works in pdfce and misbehaves elsewhere.
#[test]
...
/// for a document pdfce never touched.
#[test]
...
/// Every `/Fields` entry pdfce writes is an INDIRECT REFERENCE (§1.2.2 trap).
///
/// §12.7.3.1 requires every field to be an indirect object, and pdfce's own
/// reader collects `/Fields` roots by reference — a direct dictionary there
/// would be counted and skipped, so a field pdfce authored would be one the
/// same pdfce could not address.
#[test]
...
/// branch because it never exposes hierarchy authoring; pdfce does, so pdfce
/// needs it.
#[test]
...
/// and preserving them is the whole of pdfce's contract with them.
///
...
/// fixture in this repo — the object pdfce rewrites is the CATALOG. So the
...
/// asserting something pdfce cannot deliver while also adding a field, and a
...
/// default every widget inherits), and pdfce cannot tell which one a given
