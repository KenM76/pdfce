...
//! with a struct literal** — only through the builder. Inside `pdfce-core` that
...
//! ergonomics being shipped. These tests build it the way `pdfce-cli` has to.
//!
...
//!
//! **pdfce does not read a clock.** `/M` is the caller's string, written
...
//! this project, and a timestamp pdfce chose would be a value it *inferred* and
...
use pdfce_core::edit::{EditError, EditSession, MarkupNote, MarkupOptions};
...
fn authored(opts: &MarkupOptions) -> pdfce_core::annot::Annotation {
    let mut s = session();
...
fn an_impossible_but_well_formed_date_is_accepted() {
    ...
    // rejects this — so refusing it would be pdfce inventing a conformance
