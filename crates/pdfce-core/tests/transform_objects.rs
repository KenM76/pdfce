...
//!
//! `pdfceGUI` asked for `transform_objects` on the reasoning that
...
fn bbox_of(src: &str, index: usize) -> pdfce_core::vector::Bounds {
    let cs = ContentStream::parse(src.as_bytes().to_vec()).unwrap();
    let model = decompose(&cs, Matrix::IDENTITY, &NoXObjects);
...
/// would be pdfce choosing a shape the operator did not draw.
#[test]
