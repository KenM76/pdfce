...
//!
//! Before this Pass, exactly one class of thing in pdfce had all three of
...
fn annot_count(s: &EditSession) -> usize {
    ...
    pdfce_core::annot::page_annotations(&s.graph(), page).len()
}
...
fn a_cut_field_pastes_into_another_document() {
    use pdfce_core::formclip::{FieldPastePolicy, PasteTooltip};
