...
//!
//! Neither changes what pdfce draws. `/I` restates a selection that `/V`
//! already carries, and `/TI` is a scroll position pdfce's own list-box
...
//! pdfce and visible in Acrobat**, which renders a list box as a live control
...
use pdfce_core::edit::{ChoiceOption, EditSession, NewChoiceField};
...
fn a_selection_at_the_very_end_clamps_to_the_last_full_window() {
    ...
    let size = pdfce_core::vartext::auto_size(h);
    let visible = i64::try_from(pdfce_core::vartext::visible_line_count(h, size)).unwrap();
    assert!(
        ti <= 49 && 49 < ti + visible,
