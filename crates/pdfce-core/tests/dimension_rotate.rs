...
//! dimension displays is **identical either side of it** — not because pdfce
...
//! it 30° and the constraint no longer describes what is drawn. pdfce relaxes
...
fn first_dimension(s: &EditSession) -> (pdfce_core::dimension::DimensionId, String) {
...
fn value_of(s: &EditSession, id: pdfce_core::dimension::DimensionId) -> String {
    s.dimension_model()
        .display(id)
...
fn a_quarter_turn_puts_the_points_where_the_arithmetic_says() {
    ...
    use pdfce_core::vector::geometry::Point;
    use pdfce_core::vector::snap::AxisConstraint;
