...
//! pdfce-authored *ce dimensions*; project rule 15). Before this Pass the edit
...
//!    (`ARCHITECTURE.md` §5): pdfce rewrites what it logically changed and
//!    nothing more.
...
use pdfce_core::text_edit::{EditOptions, EditRequest, EditTarget, edit_text};
...
/// neighbour by one dictionary key, and the assertions are about what pdfce
...
/// the report: the report is pdfce's claim about what it did, and this is the
...
fn a_form_edit_rewrites_only_the_form_stream() {
    ...
    assert!(
        ...
        "the PAGE's content stream must NOT be re-emitted -- pdfce did not logically change it: {appended}"
    );
}
...
fn the_form_dictionary_is_preserved_key_for_key() {
    ...
    let obj = reloaded
        .get(pdfce_core::object::ObjId::new(5, 0))
        .map(|io| &io.value)
        .expect("object 5 is present after the edit");
    let pdfce_core::object::Object::Stream(stream) = obj else {
...
/// editing it changes both. pdfce cannot prevent that (there is exactly one
...
/// succeed on the bytes pdfce can see and never reach what is actually
...
/// would be pdfce declining to implement a rule the standard states plainly.
#[test]
...
fn targeting_the_page_contents_does_not_reach_into_a_form() {
    ...
    assert!(
        matches!(err, pdfce_core::text_edit::EditError::NoMatch(_)),
...
/// pdfce's own components rather than a rule in the standard.** Nothing
/// sanctions filling a partially-declared form's `/Font` from the page, and
/// `pdfce-render`'s interpreter does not do it: its `Do` handler takes the
