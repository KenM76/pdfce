...
use pdfce_core::attachments::{AttachmentClip, list_attachments};
...
fn a_copied_attachment_pastes_into_another_document() {
    ...
    assert_eq!(
        pdfce_core::attachments::attachment_bytes(&view, arrived),
...
fn cutting_an_attachment_is_one_undo_entry_and_undo_restores_the_bytes() {
    ...
    assert_eq!(
        back.first()
            .and_then(|a| pdfce_core::attachments::attachment_bytes(&view, a)),
        Some(b"the revision C notes".to_vec()),
