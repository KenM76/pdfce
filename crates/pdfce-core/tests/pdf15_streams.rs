...
//! files), driven only through `pdfce-core`'s **public** API
...
use pdfce_core::xref::{self, XrefEntry, XrefErrorKind};
...
fn authoring_onto_a_compressed_page_object_promotes_it_r38_x7() {
    ...
    let annots = pdfce_core::annot::page_annotations(&reloaded, pages[0].id);
