...
fn objects() -> pdfce_core::vector::PageObjects {
...
fn the_fixture_still_has_a_page_spanning_text_object() {
    ...
    let text = m
        ...
        .iter()
        .find_map(|o| match o {
            pdfce_core::vector::VectorObject::Text(t) => Some(t),
            _ => None,
        })
...
fn a_line_in_the_gap_is_the_front_most_hit() {
    ...
    assert!(
        matches!(
            m.objects.get(first),
            Some(pdfce_core::vector::VectorObject::Path(_))
        ),
...
fn a_click_on_actual_glyphs_still_hits_the_text() {
    ...
    assert!(
        hits.iter().any(|i| matches!(
            m.objects.get(*i),
            Some(pdfce_core::vector::VectorObject::Text(_))
        )),
