...
use pdfce_core::vector::edit::{anchor_count, plan_move_node, plan_move_subpath};
use pdfce_core::vector::{Matrix, NoXObjects, PathObject, Point, Segment, VectorObject, decompose};
...
fn moving_a_clipping_path_as_a_whole_discloses_it() {
    let cs = parse(b"0 0 500 700 re W n");
    let plan = pdfce_core::vector::edit::plan_move(&cs, &only_path(&cs), 10.0, 10.0)
