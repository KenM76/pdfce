...
//! answer and had already shipped against.** `pdfceGUI` restyles a pinned
...
//! emitted by `pdfce-cli extract-text --json`, so the property is unobservable
//! from outside `pdfce-core`. Answering it is owed whichever way it comes out,
...
/// **The invariant `pdfceGUI` shipped against.**
///
...
/// fixture corpus rather than of pdfce.
#[test]
