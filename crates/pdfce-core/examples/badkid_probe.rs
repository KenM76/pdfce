...
//!
//! ```text
//! cargo run -p pdfce-core --example badkid_probe            # debug_assertions ON
//! cargo run -p pdfce-core --release --example badkid_probe  # the shipping build
//! ```
//!
...
//!    unrelated reason — its base cross-reference was invalid, so pdfce
...
//! by `crates/pdfce-core/tests/annot_ap_cascade_streams.rs`, whose two hostile
...
fn annotation_ids(session: &EditSession) -> Vec<ObjId> {
    ...
    let mut out = Vec::new();
    for slot in &slots {
        for annot in pdfce_core::annot::page_annotations(&graph, slot.id) {
...
fn page_annotation_ids(session: &EditSession, page_index: usize) -> Vec<ObjId> {
    ...
    let graph = session.graph();
    pdfce_core::annot::page_annotations(&graph, slot.id)
        .into_iter()
        .filter_map(|a| a.id)
        .collect()
}
...
/// stronger one — a file pdfce itself cannot reopen is strictly worse than a
/// refusal, and it is the shape the 2026-08-20 `/Contents` corruption had.
fn observe(session: &EditSession, label: &str) {
    let walks = pdfce_core::page_tree::pages_in(&session.graph());
    ...
    let full = match session.to_full_bytes(&SaveOptions::identity()) {
        Ok((bytes, _)) => match Document::from_bytes(bytes.clone()) {
            Ok(d) => match pdfce_core::page_tree::pages_in(d.view().graph()) {
