...
//! colour space is CMYK"*. **pdfce's rasteriser targets sRGB** — `color/mod.rs`
...
//! by construction. If they disagree, patch 23.0 fails in pdfce for a reason
//! that has nothing to do with ICC.
//!
//! Run: `cargo run -p pdfce-core --example gray_equivalence_probe`

use pdfce_core::color::{cmyk_to_srgb, cmyk_to_srgb_with, gray_to_srgb};
    ...
    if d.iter().all(|v| *v == 0) {
        ...
        println!(
            "  => they DISAGREE by up to {} levels. Patch 23.0 cannot pass\n\
             \x20    while that holds, and the cause is entirely inside pdfce:\n\
